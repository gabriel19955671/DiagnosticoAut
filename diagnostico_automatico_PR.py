...
# --- CSS (Copy from your previous version) ---
st.markdown(f"""
<style>
body {{
...
# --- Funções de Gráficos ---
# (Your chart functions: create_radar_chart, create_gut_barchart, etc. - Keep as is)
...
# --- Funções Utilitárias ---
# (Keep sanitize_column_name, pdf_safe_text_output, find_client_logo_path, inicializar_csv, registrar_acao, update_user_data, carregar_analises_perguntas, obter_analise_para_resposta, gerar_pdf_diagnostico_completo as they were in the previous correct version)
# The following are copies from your previous corrected version
def sanitize_column_name(name):
    s = str(name).strip().replace(' ', '_'); s = re.sub(r'(?u)[^-\w.]', '', s); return s
def pdf_safe_text_output(text): return str(text).encode('latin-1', 'replace').decode('latin-1')
def find_client_logo_path(cnpj_arg): # This is for individual client logos on their dashboard
...
def inicializar_csv(filepath, columns, defaults=None):
    try:
        if not os.path.exists(filepath) or os.path.getsize(filepath) == 0:
            ...
            try:
                dtype_spec = {}
                if filepath in [usuarios_csv, usuarios_bloqueados_csv, arquivo_csv, notificacoes_csv, historico_csv]: # Apply to all relevant
                    dtype_spec['CNPJ'] = str
                    if 'CNPJ_Cliente' in columns: dtype_spec['CNPJ_Cliente'] = str # only if column exists
            ...
            for col_idx, col_name in enumerate(columns):
                if col_name not in current_cols:
                    ...
                    if len(df_init) > 0 and not df_init.empty:
                        ...
                        if not pd.isna(default_val) and len(df_init) == 0: # If df is empty but we have a default
                             df_init.loc[0, col_name] = default_val # Add a row for the default if file was just headers
                             df_init = df_init.iloc[0:0] # Then clear it to keep only headers for new files
    ...
    try:
        with st.spinner("Gerando PDF do diagnóstico... Aguarde."):
            ...
            if not perguntas_df.empty and "Categoria" in perguntas_df.columns:
                 # Ensure 'Categoria' is string and handle potential NaNs before calling unique()
...
if not st.session_state.admin_logado and not st.session_state.cliente_logado:
    ...
    # Display logo once before the radio button, within a centered structure
    login_form_placeholder = st.empty() # Placeholder to manage content visibility
    with login_form_placeholder.container():
        ...
        if os.path.exists(login_logo_to_display):
            st.image(login_logo_to_display, width=200) # Adjusted width
        else:
    ...
    if st.session_state.aba == "Administrador":
                ...
                if st.form_submit_button("Entrar", use_container_width=True, icon="🔑"):
                    try:
                        ...
                        if not df_creds[(df_creds["Usuario"] == u) & (df_creds["Senha"] == p)].empty:
                            ...
                            login_form_placeholder.empty() # Clear the login form
                            st.rerun()
        ...
        with login_form_placeholder.container(): # Reuse placeholder
            ...
            if os.path.exists(login_logo_to_display): # Display logo again inside the form card
                st.image(login_logo_to_display, width=180)
            ...
            st.markdown('</div>', unsafe_allow_html=True)
        st.stop()
    # If not Admin or Client selected (e.g. initial load where aba is None), stop after radio group
    ...
    aba = "Administrador" 
else: # Implies client_logado
    aba = "Cliente" 


# --- ÁREA DO CLIENTE LOGADO ---
# (This entire section should be copied from your last working version, 
# ensuring the enhancements for slots, instruction flow, and notification clicks are integrated)
# For brevity, I will only show the structure and key change points here.
if aba == "Cliente" and st.session_state.cliente_logado:
    # ... (Client sidebar, profile with updated slots, menu logic for instruction forcing) ...
    # ... (Client Page: Instruções) ...
    # ... (Client Page: Notificações - with "Ver Detalhes" button and logic) ...
    # ... (Client Page: Painel Principal - with logic to expand diagnosis based on target_diag_data_for_expansion) ...
    # ... (Client Page: Novo Diagnóstico) ...
    st.markdown("### Área do Cliente Logado")
    st.write(f"Página Atual Cliente: {st.session_state.get('cliente_page', 'N/A')}")
    st.write("Conteúdo da área do cliente (integre o código completo da versão anterior aqui, com as melhorias).")
...
if aba == "Administrador" and st.session_state.admin_logado:
    ...
    if os.path.exists(admin_logo_path_sidebar):
        st.sidebar.image(admin_logo_path_sidebar, use_container_width=True, width=150) # Control width
    else:
        st.sidebar.markdown("### Painel Admin")

    st.sidebar.success("🟢 Admin Logado")

    if st.sidebar.button("Sair do Painel Admin", icon="🚪", key="logout_admin_v19_final_btn", use_container_width=True):
    ...
    WIDGET_KEY_SB_ADMIN_MENU = "sb_admin_menu_v19_final_widget" 

    def admin_menu_on_change_final_v2(): 
        ...
        if new_text_key and new_text_key != st.session_state.get(SESSION_KEY_FOR_ADMIN_PAGE):
            ...
            # No automatic rerun from on_change, let Streamlit handle flow or trigger rerun explicitly after this block if needed.
    ...
    st.sidebar.selectbox(
        ...
        on_change=admin_menu_on_change_final_v2
    )

    # Debug: Check the current session state value for the admin page
    # st.sidebar.write(f"Debug (pre-assign): session_state[{SESSION_KEY_FOR_ADMIN_PAGE}] = {st.session_state.get(SESSION_KEY_FOR_ADMIN_PAGE)}")

    menu_admin = st.session_state[SESSION_KEY_FOR_ADMIN_PAGE] # CRITICAL: This defines menu_admin
    
    # Debug: Check the assigned value of menu_admin
    # st.sidebar.write(f"Debug (post-assign): menu_admin = {menu_admin}")


    if menu_admin not in menu_admin_options_map: # Fallback if something went very wrong
        st.error("Página administrativa inválida selecionada. Redirecionando para Visão Geral.")
        menu_admin = admin_page_text_keys[0]
        st.session_state[SESSION_KEY_FOR_ADMIN_PAGE] = menu_admin
        # Potentially rerun if state was corrected:
        # st.rerun()


    header_display_name = f"{menu_admin_options_map.get(menu_admin, '❓')} {menu_admin}" # Use .get for safety
    st.header(header_display_name)

    df_usuarios_admin_geral = pd.DataFrame(columns=colunas_base_usuarios) # Initialize
    try:
        # ... (df_usuarios_admin_geral loading and preprocessing as in previous version) ...
    ...
    # --- Admin Page Content ---
    # (The if/elif structure for menu_admin pages)
    # (Ensure this matches the structure from Turn 9, with the new pages and modifications)
    if menu_admin == "Visão Geral e Diagnósticos":
        # ... (Visão Geral logic from Turn 9, with the corrected selectbox filter)
        st.write("Conteúdo da Visão Geral e Diagnósticos (a ser preenchido com o código da v9).")
    elif menu_admin == "Relatório de Engajamento":
        # ... (New Relatório de Engajamento logic)
        st.write("Conteúdo do Relatório de Engajamento (a ser preenchido).")
    elif menu_admin == "Gerenciar Notificações":
        # ... (New Gerenciar Notificações logic)
        st.write("Conteúdo de Gerenciar Notificações (a ser preenchido).")
    elif menu_admin == "Gerenciar Clientes":
        # ... (Gerenciar Clientes logic from Turn 9, with new filter)
        st.write("Conteúdo de Gerenciar Clientes (a ser preenchido com o código da v9 e filtro).")
    elif menu_admin == "Personalizar Aparência":
        # ... (New Personalizar Aparência logic for login logo)
        ...
        2. **Logo Padrão:** Se nenhuma logo personalizada for carregada, o sistema tentará usar `{DEFAULT_LOGIN_LOGO_PATH}`. 
           Certifique-se de que este arquivo exista na pasta `{ASSETS_DIR}/` com sua logo principal (`logo PR (1).png` renomeada para `default_login_logo.png`).
        """)

        current_logo_path_for_login_admin_view = DEFAULT_LOGIN_LOGO_PATH
        status_logo_admin_view = "Padrão (default_login_logo.png)"

        if os.path.exists(CUSTOM_LOGIN_LOGO_PATH):
            current_logo_path_for_login_admin_view = CUSTOM_LOGIN_LOGO_PATH
            status_logo_admin_view = "Personalizada (portal_login_logo.png)"
        ...
        if os.path.exists(current_logo_path_for_login_admin_view):
            ...
            if current_logo_path_for_login_admin_view == CUSTOM_LOGIN_LOGO_PATH:
                if st.button("Remover Logo Personalizada e Usar Padrão", key="remove_custom_login_logo_btn_final"):
                    try:
            ...
            if current_logo_path_for_login_admin_view == DEFAULT_LOGIN_LOGO_PATH:
        ...
        uploaded_login_logo = st.file_uploader("Selecione o arquivo da nova logo:", type=["png", "jpg", "jpeg"], key="admin_login_logo_uploader_final_key")
        ...
        if uploaded_login_logo is not None:
            try:
                ...
                # Clear the uploader after successful save by changing its key implicitly via rerun or explicitly
                if "admin_login_logo_uploader_final_key" in st.session_state:
                    del st.session_state["admin_login_logo_uploader_final_key"] # One way to reset uploader state
                st.rerun() 
            except Exception as e_upload_logo:
                st.error(f"Erro ao salvar a nova logo: {e_upload_logo}")
    # ... (other admin elif blocks for Gerenciar Perguntas, Análises, Instruções, Histórico, Admins)

if not st.session_state.admin_logado and not st.session_state.cliente_logado and st.session_state.get('aba') is None:
    # Fallback if 'aba' somehow isn't set by the initial radio group.
    # This also helps avoid NameError for 'aba' if the login block logic is exited prematurely.
