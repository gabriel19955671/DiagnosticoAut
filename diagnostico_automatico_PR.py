...
def create_radar_chart(data_dict, title="Radar Chart"):
    ...
    values = [pd.to_numeric(v, errors='coerce') for v in data_dict.values()]
    values = [v for v in values if pd.notna(v)] # Remove NaNs
    categories = [cat for cat, val in zip(categories, data_dict.values()) if pd.notna(pd.to_numeric(val, errors='coerce'))]
    ...
    fig.update_layout(title={'text': title, 'x':0.5, 'xanchor': 'center'},
                        polar=dict(radialaxis=dict(visible=True, range=[0, 5])), # Ajustar range se necessário
...
def create_gut_barchart(gut_data_list, title="Top Prioridades (GUT)"):
    ...
    if df_gut.empty or not all(col in df_gut.columns for col in ["Tarefa", "Score"]):
        return None
    ...
    fig.update_layout(yaxis={'categoryorder':'total ascending'},
                        ...
                        font=dict(family="Segoe UI, sans-serif"),
                        height=max(400, 200 + len(df_gut)*30), # Ajuste dinâmico da altura
                        margin=dict(l=250, r=20, t=70, b=20)) # Aumentar margem esquerda se necessário
    return fig
...
def create_diagnostics_timeline_chart(df_diagnostics, title="Diagnósticos Realizados ao Longo do Tempo"):
    ...
    df_diag_copy = df_diagnostics.dropna(subset=['Data_dt']).copy() # Remove NaT antes de agrupar
...
def create_avg_category_scores_chart(df_diagnostics, title="Média de Scores por Categoria (Todos Clientes)"):
    ...
    for col in media_cols:
        ...
        if not numeric_scores.isnull().all(): # Garante que há pelo menos um valor não nulo
            avg_scores_data.append({
                'Categoria': col.replace("Media_Cat_", "").replace("_", " "),
                'Média_Score': numeric_scores.mean() # .mean() ignora NaNs por padrão
            })
    ...
    fig.update_layout(xaxis_tickangle=-45, font=dict(family="Segoe UI, sans-serif"),
                        yaxis=dict(range=[0,max(5.5, avg_scores['Média_Score'].max() + 0.5 if not avg_scores.empty else 5.5)])) # Ajustar range do eixo Y
    return fig
...
colunas_base_perguntas = ["Pergunta", "Categoria"] # Perguntas do diagnóstico
...
# FUNÇÃO inicializar_csv (Revisada)
def inicializar_csv(filepath, columns, defaults=None):
    try:
        dtype_spec = {}
        if filepath in [usuarios_csv, usuarios_bloqueados_csv, notificacoes_csv, sac_uso_feedback_csv, pesquisa_satisfacao_respostas_csv]:
            if 'CNPJ' in columns: dtype_spec['CNPJ'] = str
            if 'CNPJ_Cliente' in columns: dtype_spec['CNPJ_Cliente'] = str
        if filepath == arquivo_csv: # Diagnósticos
            if 'CNPJ' in columns: dtype_spec['CNPJ'] = str
            if 'Data' in columns: dtype_spec['Data'] = str # ID do diagnóstico
        if filepath == pesquisa_satisfacao_respostas_csv:
            if 'ID_Diagnostico_Cliente' in columns: dtype_spec['ID_Diagnostico_Cliente'] = str
        ...
        if not os.path.exists(filepath) or os.path.getsize(filepath) == 0:
            ...
            try:
                ...
                st.error(f"Erro crítico ao ler {filepath}: {read_e}. Verifique o arquivo. Tentando continuar com DataFrame vazio.")
                return pd.DataFrame(columns=columns) # Retorna vazio para evitar mais erros
        ...
        st.error(f"Erro fatal na função inicializar_csv para {filepath}: {e}")
        st.stop() # Parar a execução se a inicialização falhar criticamente
        return pd.DataFrame(columns=columns) # Não deve chegar aqui devido ao st.stop()
...
try:
    ...
    inicializar_csv(sac_uso_feedback_csv, colunas_base_sac_uso_feedback, defaults={"Feedback_Util": None}) # pd.NA não é bom para CSV, usar string ou deixar None
...
# FUNÇÕES DE CARREGAMENTO DE DADOS (revisadas e adicionadas)
...
def carregar_sac_uso_feedback():
    try:
        ...
        if 'Feedback_Util' in df.columns:
             # Convertendo para boolean, tratando strings 'true'/'false' e nulos
            ...
            df['Feedback_Util'] = pd.Series(dtype='boolean') # Cria coluna boolean vazia
        return df
    except (FileNotFoundError, pd.errors.EmptyDataError):
...
def carregar_perguntas_satisfacao(apenas_ativas=True):
    try:
        ...
        else: df['Ativa'] = df['Ativa'].fillna(True).astype(bool) # Garante boolean, preenche NaN com True
        if 'Ordem' not in df.columns: df['Ordem'] = 0
        else: df['Ordem'] = pd.to_numeric(df['Ordem'], errors='coerce').fillna(0).astype(int)
        if 'Opcoes_Pergunta' not in df.columns: df['Opcoes_Pergunta'] = None
    ...
    for _, row_analise in analises_da_pergunta.iterrows():
        ...
        try:
            if tipo_cond == 'FaixaNumerica':
                min_val=pd.to_numeric(row_analise.get('CondicaoValorMin'),errors='raise')
                max_val=pd.to_numeric(row_analise.get('CondicaoValorMax'),errors='raise')
                resp_num=pd.to_numeric(resposta_valor,errors='raise')
                ...
                min_s=pd.to_numeric(row_analise.get('CondicaoValorMin'),errors='raise')
                max_s_raw=row_analise.get('CondicaoValorMax')
                max_s=pd.to_numeric(max_s_raw,errors='coerce') if pd.notna(max_s_raw) else pd.NA # Trata max_s como opcional
                resp_s_gut=pd.to_numeric(resposta_valor,errors='raise')

                is_min = resp_s_gut >= min_s
                is_max_ok = pd.isna(max_s) or (resp_s_gut <= max_s) # Se max_s for NA, considera ok
                if is_min and is_max_ok: return analise_txt
        except (ValueError, TypeError): continue
    return default_analise

# FUNÇÃO GERAR PDF (revisada)
def gerar_pdf_diagnostico_completo(diag_data, user_data, perguntas_df_diagnostico, respostas_coletadas, medias_cat, analises_df):
    try:
        with st.spinner("Gerando PDF do diagnóstico... Aguarde."):
            ...
            pdf.ln(3)
            mg_pdf = diag_data.get('Média Geral')
            gut_pdf = diag_data.get('GUT Média')
            ...
            if perguntas_df_diagnostico.empty or "Pergunta" not in perguntas_df_diagnostico.columns or "Categoria" not in perguntas_df_diagnostico.columns:
                ...
                categorias = perguntas_df_diagnostico["Categoria"].unique()
                for categoria in categorias:
                    pdf.set_font("Arial", 'B', 10); pdf.multi_cell(0, 7, pdf_safe_text_output(f"Categoria: {categoria}")); pdf.set_font("Arial", size=9)
                    perg_cat = perguntas_df_diagnostico[perguntas_df_diagnostico["Categoria"] == categoria]
    ...
    with st.form("form_cliente_login_v19"):
        ...
        if st.form_submit_button("Entrar", use_container_width=True, icon="👤"):
            ...
            try:
                ...
                users_df["JaVisualizouInstrucoes"] = users_df["JaVisualizouInstrucoes"].astype(str).str.lower().map({'true': True, 'false': False, 'nan': False, '':False, 'none':False}).fillna(False)
                ...
                st.session_state.progresso_diagnostico_percentual = 0; st.session_state.progresso_diagnostico_contagem = (0,0)
                ...
                st.session_state.respostas_pesquisa_satisfacao_atual = {}; st.session_state.pesquisa_satisfacao_enviada = False
...
if aba == "Cliente" and st.session_state.cliente_logado:
    ...
    if os.path.exists(notificacoes_csv):
        ...
        except: pass # Silencioso se falhar a contagem
    ...
    current_idx_cli = menu_options_cli_display.index(default_display_option) if default_display_option and default_display_option in menu_options_cli_display else 0
    if not menu_options_cli_display: current_idx_cli = 0 # Fallback
    else:
        if current_idx_cli >= len(menu_options_cli_display) or not default_display_option: # Se o índice for inválido ou opção não existir mais
            current_idx_cli = 0
            st.session_state.cliente_page = [k for k,v in menu_options_cli_map.items() if v == menu_options_cli_display[0]][0]


    selected_page_cli_raw = st.sidebar.radio("Menu Cliente", menu_options_cli_display, index=current_idx_cli, key="cli_menu_v19_final")
    ...
    if st.sidebar.button("Sair do Portal Cliente", icon="⬅️", key="logout_cliente_v19_final", use_container_width=True):
        # ... (lógica de logout cliente)
        for key_item in list(st.session_state.keys()): # Itera sobre cópia das chaves
            if key_item not in ['admin_logado', 'last_cnpj_input']: del st.session_state[key_item]
        for key_d, value_d in default_session_state.items(): # Re-inicializa defaults
    ...
    if st.session_state.cliente_page == "Instruções":
        ...
        if df_sac_qa.empty: st.info("Nenhuma pergunta frequente cadastrada.")
        else: # ... (lógica de exibição e feedback do SAC)
            df_sac_qa_sorted = df_sac_qa.sort_values(by=["Categoria_SAC", "Pergunta_SAC"])
            categorias_sac = df_sac_qa_sorted["Categoria_SAC"].unique()
            search_term_sac = st.text_input("🔎 Procurar nas Perguntas Frequentes:", key="search_sac_cliente_final")
            if search_term_sac: # ... (lógica de busca)
                pass
            for categoria in categorias_sac: # ... (loop de exibição)
                pass

    elif st.session_state.cliente_page == "Notificações":
        # ... (código página Notificações)
        st.subheader(menu_options_cli_map_full["Notificações"].split(" (")[0])
        # ... (lógica de exibição e marcação de notificações como lidas)
        ...
        if st.session_state.diagnostico_enviado_sucesso:
            ...
            if st.session_state.pdf_gerado_path and st.session_state.pdf_gerado_filename:
                try:
                    with open(st.session_state.pdf_gerado_path, "rb") as f_pdf:
                        st.download_button(label="Baixar PDF do Diagnóstico Recém-Enviado", data=f_pdf, file_name=st.session_state.pdf_gerado_filename, mime="application/pdf", key="dl_novo_diag_painel_v19_final", icon="📄")
                    st.session_state.pdf_gerado_path = None; st.session_state.pdf_gerado_filename = None
                except Exception as e_pdf_dl: st.warning(f"Erro ao disponibilizar PDF para download: {e_pdf_dl}")
            ...
            if st.session_state.id_diagnostico_concluido_para_satisfacao and not st.session_state.pesquisa_satisfacao_enviada:
                ...
                if verificar_pesquisa_satisfacao_pendente(st.session_state.cnpj, id_diag_atual_satisfacao):
                    ...
                    # ... (lógica da pesquisa de satisfação - como na resposta anterior)
                    perguntas_satisfacao_df = carregar_perguntas_satisfacao(apenas_ativas=True)
                    if not perguntas_satisfacao_df.empty:
                         with st.form(key="form_pesquisa_satisfacao_cliente_final"):
                            # ... (renderização das perguntas da pesquisa)
                            if st.form_submit_button("Enviar Feedback de Satisfação", use_container_width=True, icon="💖"):
                                # ... (salvamento das respostas da pesquisa)
                                st.cache_data.clear(); st.session_state.pesquisa_satisfacao_enviada = True; st.rerun()
                elif not st.session_state.pesquisa_satisfacao_enviada : # Já respondeu, mas não nesta sessão de visualização
                     st.success("Você já respondeu à pesquisa de satisfação para este diagnóstico. Obrigado!")
            st.session_state.diagnostico_enviado_sucesso = False # Reset da flag
        ...
        try:
            ...
            for col_num in cols_numericas_diag:
                if col_num in df_antigos_cliente.columns: df_antigos_cliente[col_num] = pd.to_numeric(df_antigos_cliente[col_num], errors='coerce')
            df_cliente_diags_raw = df_antigos_cliente[df_antigos_cliente["CNPJ"] == st.session_state.cnpj].copy()
        except Exception as e_load_diag_cli: st.error(f"Erro ao carregar históricos de diagnóstico: {e_load_diag_cli}")
        ...
        if not df_cliente_diags_raw.empty:
            ...
            # ... (Restante da exibição do Painel Principal: gráficos, diagnósticos anteriores, etc.)
        else:
        ...
        # ... (código da página Novo Diagnóstico - crucial salvar e setar 'id_diagnostico_concluido_para_satisfacao')
        ...
        # ... (resto do formulário de diagnóstico)
        if st.button("Concluir e Enviar Diagnóstico", key="enviar_diag_final_cliente_v19_final", icon="✔️", use_container_width=True):
            # ... (lógica de envio e salvamento do diagnóstico)
            id_diagnostico_salvo = st.session_state.id_formulario_atual # ID do diagnóstico
            # ... (preparar nova_linha_diag_final_n)
            # Salvar no CSV
            # ...
            st.session_state.id_diagnostico_concluido_para_satisfacao = id_diagnostico_salvo # IMPORTANTE
            st.session_state.diagnostico_enviado_sucesso = True
            st.session_state.pesquisa_satisfacao_enviada = False # Reset para nova pesquisa
            st.session_state.cliente_page = "Painel Principal"
            st.rerun()

# --- ÁREA DO ADMINISTRADOR LOGADO ---
if aba == "Administrador" and st.session_state.admin_logado:
    # ... (código da sidebar do admin, selectbox do menu, e definição de `menu_admin` - como na correção anterior)
    # ... (Carregamento de df_usuarios_admin_geral)

    # --- SEÇÕES DO ADMIN ---
    if menu_admin == "Visão Geral e Diagnósticos":
        # ... (código da Visão Geral, usando diagnosticos_df_admin_orig_view com Data e Data_dt)
        pass
    elif menu_admin == "Relatório de Engajamento":
        # ... (código do Relatório de Engajamento)
        pass
    elif menu_admin == "Gerenciar Notificações":
        # ... (código de Gerenciar Notificações)
        pass
    elif menu_admin == "Gerenciar Clientes":
        # ... (código de Gerenciar Clientes)
        pass
    elif menu_admin == "Gerenciar Perguntas (Diagnóstico)":
        # ... (código de Gerenciar Perguntas do Diagnóstico)
        pass
    elif menu_admin == "Gerenciar Análises de Perguntas":
        # ... (código de Gerenciar Análises)
        pass
    elif menu_admin == "Gerenciar SAC":
        # ... (código de Gerenciar SAC)
        pass
    elif menu_admin == "⭐ Gerenciar Pesquisa de Satisfação":
        # ... (código de Gerenciar Perguntas da Pesquisa de Satisfação)
        pass
    elif menu_admin == "📊 Relatório Pesquisa de Satisfação":
        # ... (código do Relatório da Pesquisa de Satisfação)
        pass
    elif menu_admin == "Gerenciar Instruções":
        # ... (código de Gerenciar Instruções)
        pass
    elif menu_admin == "Histórico de Usuários":
        # ... (código do Histórico de Usuários)
        pass
    elif menu_admin == "Gerenciar Administradores":
        # ... (código de Gerenciar Administradores)
        pass
