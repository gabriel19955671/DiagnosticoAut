...
st.markdown("""
<style>
/* .login-container CSS (no longer used for login, but kept for potential future use or other modals) */
...
st.title("Portal de Diagnóstico Empresarial") # Removed lock icon

# --- Configuração de Arquivos e Variáveis Globais ---
admin_credenciais_csv = "admins.csv" # Still used for admin management, not login
usuarios_csv = "usuarios.csv"
arquivo_csv = "diagnosticos_clientes.csv"
usuarios_bloqueados_csv = "usuarios_bloqueados.csv" # For admin to block clients
...
default_session_state = {
    # Removed admin_logado, cliente_logado as primary flags from here
    "diagnostico_enviado_sucesso": False,
    "inicio_sessao_cliente": None, # Will be set on client selection
    "cliente_page": "Instruções", # Default page for a selected client
    "cnpj": None, # Will be set on client selection
    "user": None, # Will be set on client selection
    ...
    "current_app_mode": "Visualização Cliente", # Default mode
    "selected_client_identifier_for_view": None # Stores "Empresa (CNPJ)"
}
...
def inicializar_csv(filepath, columns, defaults=None):
    try:
        if not os.path.exists(filepath) or os.path.getsize(filepath) == 0:
            ...
            try:
                # Specify dtype for CNPJ columns to ensure they are read as strings
                dtype_spec = None
                if filepath in [usuarios_csv, usuarios_bloqueados_csv, arquivo_csv, notificacoes_csv, historico_csv]:
                    dtype_spec = {col: str for col in ['CNPJ', 'CNPJ_Cliente'] if col in columns}

                df_init = pd.read_csv(filepath, encoding='utf-8', dtype=dtype_spec)

                # Check if DataFrame is truly empty (e.g. only headers after read)
                if df_init.empty and os.path.getsize(filepath) > 0: # Header only
                     df_init = pd.DataFrame(columns=columns) # Recreate with proper columns
                     if defaults:
                         for col, default_val in defaults.items():
                             if col in columns: df_init[col] = default_val
                     df_init.to_csv(filepath, index=False, encoding='utf-8')
                     return # Exit after re-initializing header-only file

            except pd.errors.EmptyDataError: # CSV exists but pandas thinks it's empty (e.g. just newline)
                df_init = pd.DataFrame(columns=columns)
                if defaults:
                    for col, default_val in defaults.items():
                        if col in columns: df_init[col] = default_val
                df_init.to_csv(filepath, index=False, encoding='utf-8')
                return # Exit after initializing an effectively empty file
            ...
            for col_idx, col_name in enumerate(expected_cols_df.columns):
                if col_name not in df_init.columns:
                    default_val_insert = defaults.get(col_name, pd.NA) if defaults else pd.NA
                    # Ensure inserted column has a compatible type if possible, or fill with NA then convert
                    if default_val_insert is pd.NA and len(df_init) > 0:
                         df_init.insert(loc=min(col_idx, len(df_init.columns)), column=col_name, value=[default_val_insert] * len(df_init))
                    elif default_val_insert is not pd.NA :
                         df_init.insert(loc=min(col_idx, len(df_init.columns)), column=col_name, value=default_val_insert)
                    else: # df_init is empty, insert column with default
                         df_init[col_name] = default_val_insert
        ...
        st.error(f"Erro de parsing ao ler {filepath}. O arquivo pode estar corrompido: {pe}")
        # Optionally, create a blank one to prevent further errors
        df_init = pd.DataFrame(columns=columns)
        if defaults:
            for col, default_val in defaults.items():
                if col in columns: df_init[col] = default_val
        ...
        # Depending on severity, you might want to st.stop() or raise
        # For now, we'll try to create a blank file if it fails badly
        try:
            df_init = pd.DataFrame(columns=columns)
            if defaults:
                for col, default_val in defaults.items():
            ...
            st.error(f"Falha ao tentar recriar {filepath} após erro: {e_final}")
            raise # Re-raise if even recreation fails

try:
    inicializar_csv(usuarios_bloqueados_csv, ["CNPJ"])
    inicializar_csv(admin_credenciais_csv, ["Usuario", "Senha"]) # For admin management
    ...
    inicializar_csv(arquivo_csv, colunas_base_diagnosticos) # Ensure this file exists
...
""")
    ...
    st.error(f"Erro fatal na inicialização de arquivos CSV ou diretórios: {e}")
    st.exception(e)
    st.stop()
...
def update_user_data(cnpj, field, value):
    try:
        ...
        if not idx.empty:
            ...
            # Update session state user if the current user is being modified
...
def obter_analise_para_resposta(pergunta_texto, resposta_valor, df_analises):
    ...
    for _, row_analise in analises_da_pergunta.iterrows():
        ...
        if tipo_cond == 'FaixaNumerica':
            ...
            is_max_ok = pd.isna(max_s) or (pd.notna(resp_s_gut) and resp_s_gut <= max_s) # Max can be NA for "above min"
...
def gerar_pdf_diagnostico_completo(diag_data, user_data, perguntas_df, respostas_coletadas, medias_cat, analises_df):
    try:
        ...
        if not perguntas_df.empty:
            for _, p_row in perguntas_df.iterrows():
                ...
                if "[Matriz GUT]" in p_texto:
                    ...
                    else: continue # No action if score is 0
        ...
        return pdf_path
    except Exception as e: st.error(f"Erro crítico ao gerar PDF de diagnóstico: {e}"); st.exception(e); return None
...
def gerar_pdf_historico(df_historico_filtrado, titulo="Histórico de Ações"):
    ...
    col_widths = {"Data": 35, "CNPJ": 35, "Ação": 40, "Descrição": 75} # Adjusted Descrição width slightly
    ...
    for _, row in df_historico_filtrado.iterrows():
        ...
        # Pre-calculate height for the description cell if it exists
        desc_text_hist = str(row.get("Descrição", "")) if "Descrição" in row else ""
        if desc_text_hist:
            # Use a temporary PDF instance or a non-rendering call if available in your FPDF version
            # to calculate the number of lines the description will take.
            # For standard PyFPDF, this is tricky. We might estimate or use a fixed max height.
            # A simple estimation:
            lines = 1
            if len(desc_text_hist) > (col_widths.get("Descrição", 75) / 2): # Crude estimation factor
                 # This is a very rough way to estimate lines.
                 # FPDF2's split_only=True is better if available.
                 # For simplicity, let's assume max 3-4 lines or calculate based on character count.
                 lines = pdf.multi_cell(col_widths.get("Descrição", 75), 5, pdf_safe_text_output(desc_text_hist), border=0, align="L", dry_run=True, output="LINES") if hasattr(pdf, 'multi_cell') and 'dry_run' in pdf.multi_cell.__code__.co_varnames else (len(desc_text_hist) // (col_widths.get("Descrição", 75)//3)) +1


            max_h_row_hist = max(max_h_row_hist, lines * 5) # Assuming line height of 5
        ...
        for header_idx, header in enumerate(headers_to_print_hist):
            ...
            # CRITICAL FIX: Removed ln=0 from multi_cell
        ...
        pdf.set_y(current_y_hist + max_h_row_hist) # Move Y after the entire row is drawn
...
def reset_client_session_state():
    st.session_state.diagnostico_enviado_sucesso = False
    st.session_state.inicio_sessao_cliente = None
    st.session_state.cliente_page = "Instruções" # Default for new selection
    ...
    st.session_state.id_formulario_atual = None # Will be regenerated
    ...
    st.session_state.selected_client_identifier_for_view = None # Clear selection
    if 'notif_page_loaded_once_v14_final_c' in st.session_state:
        del st.session_state.notif_page_loaded_once_v14_final_c
    st.rerun()


# --- Main Application Flow (No Login) ---
st.sidebar.header("Menu de Navegação")
app_mode = st.sidebar.radio(
    "Selecione o Modo de Operação:",
    ("Visualização Cliente", "Painel Administrativo"),
    key="main_app_mode_selector_v2",
    on_change=lambda: setattr(st.session_state, 'selected_client_identifier_for_view', None) # Reset client on mode change
)
st.session_state.current_app_mode = app_mode


if st.session_state.current_app_mode == "Visualização Cliente":
    st.sidebar.markdown("---")
    try:
        usuarios_df_for_selection = pd.read_csv(usuarios_csv, dtype={'CNPJ': str}, encoding='utf-8')
        if usuarios_df_for_selection.empty:
            st.sidebar.warning("Nenhum cliente cadastrado para seleção.")
            st.warning("Não há clientes cadastrados no sistema. Por favor, adicione clientes através do Painel Administrativo.")
            st.stop()

        client_options = ["-- Selecione um Cliente --"] + sorted(
            usuarios_df_for_selection.apply(lambda row: f"{row['Empresa']} ({row['CNPJ']})", axis=1).tolist()
        )
        
        # Preserve selection across reruns unless explicitly changed or mode switched
        if 'selected_client_identifier_for_view' not in st.session_state:
            st.session_state.selected_client_identifier_for_view = client_options[0]

        selected_client_display = st.sidebar.selectbox(
            "Selecione o Cliente:",
            client_options,
            index=client_options.index(st.session_state.selected_client_identifier_for_view) if st.session_state.selected_client_identifier_for_view in client_options else 0,
            key="client_selector_sidebar_v2"
        )

        if selected_client_display != st.session_state.selected_client_identifier_for_view :
            st.session_state.selected_client_identifier_for_view = selected_client_display
             # Reset parts of client state when selection changes
        ...
        if selected_client_display != "-- Selecione um Cliente --":
            try:
                cnpj_selected = re.search(r'\((.*?)\)', selected_client_display).group(1)
                user_data_selected = usuarios_df_for_selection[usuarios_df_for_selection['CNPJ'] == cnpj_selected].iloc[0].to_dict()

                # Set user context in session state
                st.session_state.cnpj = cnpj_selected
                st.session_state.user = user_data_selected
                ...
                if st.session_state.id_formulario_atual is None or not st.session_state.id_formulario_atual.startswith(cnpj_selected):
                    st.session_state.id_formulario_atual = f"{cnpj_selected}_{datetime.now().strftime('%Y%m%d%H%M%S%f')}"


                # --- This is the start of the original "CLIENTE LOGADO" area ---
                # Ensure user data is loaded for the selected client
                if "user" not in st.session_state or st.session_state.user is None or \
                   "cnpj" not in st.session_state or st.session_state.cnpj is None:
                    st.error("Erro ao carregar dados do cliente selecionado. Tente selecionar novamente.")
                    st.stop()

                st.sidebar.markdown(f"### Cliente: {st.session_state.user.get('Empresa', 'Cliente')} 👋")
                ...
                if instrucoes_pendentes_obrigatorias_val and st.session_state.cliente_page != "Instruções":
                    effective_cliente_page = "Instruções" # Force to instructions if pending
                    if st.session_state.cliente_page != "Instruções": # only update if it's not already instructions
                        st.session_state.cliente_page = "Instruções"
                        # No rerun here, let the radio button selection handle it if user changes
                ...
                try: current_idx_cli_val = menu_options_cli_val.index(current_page_for_radio)
                    ...
                    current_idx_cli_val = 0 # Default to "Instruções"
                    st.session_state.cliente_page = "Instruções"

                selected_page_cli_raw_val = st.sidebar.radio("Menu Cliente", menu_options_cli_val, index=current_idx_cli_val, key="cli_menu_v16_no_login")
                ...
                if selected_page_cli_actual != st.session_state.cliente_page:
                    if instrucoes_pendentes_obrigatorias_val and selected_page_cli_actual != "Instruções":
                        ...
                        # Do not change page, keep them on current (which should be instructions if forced)
                    else:
                        st.session_state.cliente_page = selected_page_cli_actual
                        st.rerun()

                if st.sidebar.button("↩️ Limpar Seleção de Cliente", key="deselect_cliente_v16"):
                    reset_client_session_state() # This will rerun

                # --- Conteúdo da Página do Cliente (original logic) ---
                if st.session_state.cliente_page == "Instruções":
                    st.subheader("📖 Instruções do Sistema de Diagnóstico")
                    default_instructions_text = """**Bem-vindo ao Portal de Diagnóstico Empresarial!** (Default text if file is missing)"""
                    ...
                    if st.session_state.user:
                        ...
                        if pode_fazer_novo_inst_page_val:
                            # Use st.session_state.confirmou_instrucoes_checkbox_cliente for the widget's state
                            confirmou_local = st.checkbox(
                                "Declaro que li e compreendi todas as instruções fornecidas para a realização deste diagnóstico.",
                                value=st.session_state.confirmou_instrucoes_checkbox_cliente, 
                                key="confirma_leitura_inst_v16_cb"
                            )
                            st.session_state.confirmou_instrucoes_checkbox_cliente = confirmou_local # Update session state from checkbox

                            if st.button("Prosseguir para o Diagnóstico", key="btn_instrucoes_v16_prosseguir", disabled=not st.session_state.confirmou_instrucoes_checkbox_cliente):
                                if st.session_state.confirmou_instrucoes_checkbox_cliente:
                                    ...
                                    if st.session_state.user: st.session_state.user["ConfirmouInstrucoesParaSlotAtual"] = True
                                    st.session_state.cliente_page = "Novo Diagnóstico"
                                    st.session_state.confirmou_instrucoes_checkbox_cliente = False # Reset for next time if needed
                                    st.rerun()
                        else:
                            st.info("Você não possui diagnósticos disponíveis no momento.")
                            if st.button("Ir para o Painel Principal", key="ir_painel_inst_sem_diag_v16"): 
                                st.session_state.cliente_page = "Painel Principal"; st.rerun()
                    else: st.error("Erro de sessão do usuário. Por favor, selecione um cliente novamente.")
                        ...
                        if st.session_state.pdf_gerado_path and st.session_state.pdf_gerado_filename:
                            try:
                                with open(st.session_state.pdf_gerado_path, "rb") as f_pdf:
                                    st.download_button(label="📄 Baixar PDF do Diagnóstico Recém-Enviado", data=f_pdf, file_name=st.session_state.pdf_gerado_filename, mime="application/pdf", key="dl_novo_diag_painel_v16_pp")
                            except FileNotFoundError:
                                st.error("Arquivo PDF do diagnóstico recente não encontrado. Pode ter sido removido.")
                            finally: # Clear these regardless of download success to prevent stale links
                                st.session_state.pdf_gerado_path = None
                                st.session_state.pdf_gerado_filename = None
                    ...
                    try:
                        if st.session_state.get("cnpj") is None:
                            st.error("Erro: CNPJ do cliente não identificado. Não é possível carregar o painel.")
                        elif not os.path.exists(arquivo_csv):
                            st.warning(f"Arquivo de diagnósticos ('{arquivo_csv}') não encontrado.")
                        else:
                            # Ensure CNPJ is read as string from diagnósticos_clientes.csv
                            ...
                            else: # File exists but is empty or only headers
                                st.info(f"O arquivo de diagnósticos ('{arquivo_csv}') está vazio. Nenhum diagnóstico para exibir.")
                    except pd.errors.EmptyDataError: 
                        st.info(f"O arquivo de diagnósticos ('{arquivo_csv}') está vazio ou contém apenas cabeçalhos.")
                    ...
                    if df_cliente_diags.empty:
                        ...
                        try:
                            ...
                            else: st.warning(f"Arquivo de perguntas '{perguntas_csv}' não encontrado.")
                        ...
                        for idx_row_diag_orig, row_diag_data_series in df_cliente_diags.iterrows():
                            # idx_row_diag_orig is the original index from df_antigos
                            row_diag_data = row_diag_data_series.to_dict() # Work with dict for easier .get
                            diag_date_str = row_diag_data.get('Data', f"idx_{idx_row_diag_orig}")
                            emp_name_str = row_diag_data.get('Empresa', 'N/A')

                            with st.expander(f"📅 {diag_date_str} - {emp_name_str}"):
                                ...
                                if not perguntas_df_para_painel.empty:
                                    for cat_loop in sorted(perguntas_df_para_painel["Categoria"].unique()):
                                        ...
                                        for _, p_row_loop in perg_cat_loop.iterrows():
                                            ...
                                            if "[Matriz GUT]" in p_texto_loop:
                                                ...
                                                if isinstance(resp_loop, dict): g,u,t=int(resp_loop.get("G",0)),int(resp_loop.get("U",0)),int(resp_loop.get("T",0))
                                                    ...
                                                    except: pass # Silently ignore parse error for old data
                                        ...
                                        st.markdown("---") # End of category
                                else: st.caption("Estrutura de perguntas não carregada.")
                                
                                analise_cli_val_cv_painel = row_diag_data.get("Análise do Cliente", ""); 
                                analise_cli_cv_input = st.text_area("🧠 Minha Análise sobre este Diagnóstico:", value=analise_cli_val_cv_painel, key=f"analise_cv_painel_v16_pp_{idx_row_diag_orig}")
                                if st.button("💾 Salvar Minha Análise", key=f"salvar_analise_cv_painel_v16_pp_{idx_row_diag_orig}"):
                                    try:
                                        df_antigos_upd = pd.read_csv(arquivo_csv, encoding='utf-8', dtype={'CNPJ':str})
                                        # Use original index to locate the row
                                        df_antigos_upd.loc[idx_row_diag_orig, "Análise do Cliente"] = analise_cli_cv_input
                                        df_antigos_upd.to_csv(arquivo_csv, index=False, encoding='utf-8')
                                        registrar_acao(st.session_state.cnpj, "Análise Cliente (Edição Painel)", f"Editou análise do diagnóstico de {row_diag_data.get('Data')}")
                                ...
                                if com_admin_val_cv_painel and not pd.isna(com_admin_val_cv_painel): st.markdown("**Comentários do Consultor:**"); st.info(f"{com_admin_val_cv_painel}")
                                else: st.caption("Nenhum comentário do consultor para este diagnóstico.")
                                
                                if st.button("📄 Baixar PDF deste Diagnóstico", key=f"dl_pdf_antigo_v16_pp_{idx_row_diag_orig}"):
                                    medias_cat_pdf_antigo = {k.replace("Media_Cat_","").replace("_"," "):v for k,v in row_diag_data.items() if isinstance(k, str) and "Media_Cat_" in k and pd.notna(v)}
                                    pdf_path_antigo = gerar_pdf_diagnostico_completo(row_diag_data, st.session_state.user, perguntas_df_para_painel, row_diag_data, medias_cat_pdf_antigo, analises_df_para_painel)
                                    if pdf_path_antigo:
                                        with open(pdf_path_antigo, "rb") as f_antigo: 
                                            st.download_button("Download PDF Confirmado", f_antigo, file_name=f"diag_{sanitize_column_name(row_diag_data.get('Empresa','ND'))}_{str(row_diag_data.get('Data','')).replace(':','-').replace(' ','_')}.pdf", mime="application/pdf", key=f"dl_confirm_antigo_v16_pp_{idx_row_diag_orig}")
                                        registrar_acao(st.session_state.cnpj, "Download PDF (Painel)", f"Baixou PDF de {row_diag_data.get('Data')}")
                                    else: st.error("Erro ao gerar PDF.")
                                st.divider() # End of expander
                        
                        st.subheader("📌 Plano de Ação - Kanban (Baseado no Último Diagnóstico)")
                        if not df_cliente_diags.empty:
                            latest_diag_kanban_series = df_cliente_diags.iloc[0]; latest_diag_kanban = latest_diag_kanban_series.to_dict(); gut_cards_kanban = []
                            for pergunta_k, resposta_k_str in latest_diag_kanban.items():
                                if isinstance(pergunta_k, str) and "[Matriz GUT]" in pergunta_k:
                                    ...
                                    except (json.JSONDecodeError, ValueError, TypeError) as e_kanban_painel: st.warning(f"Erro ao processar GUT para Kanban '{pergunta_k}': {e_kanban_painel}", icon="⚠️")
                            if gut_cards_kanban: # Check if list is not empty
                            ...
                            else: st.info("Nenhuma tarefa GUT (>0) encontrada no último diagnóstico para o Kanban.")
                        ...
                        if len(df_cliente_diags) > 1:
                            df_evolucao = df_cliente_diags.sort_values(by="Data").copy(); df_evolucao["Data"] = pd.to_datetime(df_evolucao["Data"]); cols_plot_evol = ['Média Geral', 'GUT Média']
                            for col_ev in df_evolucao.columns:
                                if isinstance(col_ev, str) and str(col_ev).startswith("Media_Cat_"):
                        ...
                        st.divider()
                        st.subheader("📊 Comparação Detalhada Entre Diagnósticos (Radar e Tabela)")
                        if len(df_cliente_diags) >= 1:
                            datas_opts_comp_list = df_cliente_diags["Data"].astype(str).tolist(); default_selection_comp = datas_opts_comp_list[:min(2, len(datas_opts_comp_list))] # Select latest 1 or 2
                            diagnosticos_selecionados_comp = st.multiselect("Selecione os diagnósticos para comparar (máx 3 para radar):", options=datas_opts_comp_list, default=default_selection_comp, key="cliente_multiselect_comparacao_v16_pp")
                            if len(diagnosticos_selecionados_comp) >= 1:
                                ...
                                if len(diagnosticos_selecionados_comp) in [1,2,3] and not perguntas_df_para_painel.empty:
                                    ...
                                    if categorias_radar:
                                        for _, diag_row_comp_series in df_comparacao_cliente.iterrows():
                                            diag_row_comp = diag_row_comp_series.to_dict()
                                            ...
                                            values_radar = [medias_cat_comp.get(cat, 0) if pd.notna(medias_cat_comp.get(cat,0)) else 0 for cat in categorias_radar] # ensure 0 for NaN
                                            fig_radar_comp.add_trace(go.Scatterpolar(r=values_radar, theta=categorias_radar, fill='toself', name=f"Diag. {pd.to_datetime(diag_row_comp.get('Data')).strftime('%d/%m/%y')}"))
                                ...
                                metricas_interesse_comp = ["Média Geral", "GUT Média"] + [col for col in df_comparacao_cliente.columns if isinstance(col,str) and col.startswith("Media_Cat_")]
                                ...
                                if "Data" in df_comparacao_cliente and len(cols_para_pivot) > 1:
                                    ...
                                    try: df_pivot_comp.columns = [pd.to_datetime(col).strftime('%d/%m/%y %H:%M') for col in df_pivot_comp.columns]
                    ...
                    if not st.session_state.user: st.error("Erro: Dados do usuário não carregados. Selecione um cliente."); st.stop()
                    ...
                    if not pode_fazer_novo_form:
                        ...
                        if st.button("Voltar ao Painel Principal", key="voltar_painel_novo_diag_bloq_v16_nd"): st.session_state.cliente_page = "Painel Principal"; st.rerun()
                        st.stop()
                    elif not confirmou_inst_form:
                        st.warning("Por favor, confirme a leitura das instruções antes de iniciar um novo diagnóstico.")
                        if st.button("Ir para Instruções", key="ir_instrucoes_novo_diag_v16_nd"): st.session_state.cliente_page = "Instruções"; st.rerun()
                        st.stop()
                    
                    if st.session_state.diagnostico_enviado_sucesso: # Handle display after successful submission
                        st.success("🎯 Seu diagnóstico foi enviado com sucesso!")
                        if st.session_state.pdf_gerado_path and st.session_state.pdf_gerado_filename:
                            try:
                                with open(st.session_state.pdf_gerado_path, "rb") as f_pdf_dl_sucesso:
                                    st.download_button(label="📄 Baixar PDF do Diagnóstico Enviado", data=f_pdf_dl_sucesso, file_name=st.session_state.pdf_gerado_filename, mime="application/pdf", key="dl_pdf_sucesso_novo_diag_v16_nd")
                            except FileNotFoundError:
                                st.error("Arquivo PDF do diagnóstico enviado não encontrado. Pode ter sido removido.")
                            # Clear path and filename after attempting download to prevent stale state
                            st.session_state.pdf_gerado_path = None
                            st.session_state.pdf_gerado_filename = None
                        
                        if st.button("Ir para o Painel Principal", key="ir_painel_apos_envio_sucesso_v16_nd"):
                            st.session_state.cliente_page = "Painel Principal"
                            st.session_state.diagnostico_enviado_sucesso = False # Reset flag
                            st.rerun()
                        st.stop() # Stop further rendering of the form page

                    perguntas_df_formulario = pd.DataFrame()
                    try:
                        if not os.path.exists(perguntas_csv): st.error(f"Arquivo de perguntas ('{perguntas_csv}') não encontrado."); st.stop()
                        perguntas_df_formulario = pd.read_csv(perguntas_csv, encoding='utf-8')
                        if perguntas_df_formulario.empty: st.warning("Nenhuma pergunta cadastrada no sistema."); st.stop()
                        if "Categoria" not in perguntas_df_formulario.columns: perguntas_df_formulario["Categoria"] = "Geral"
                    except pd.errors.EmptyDataError: st.error(f"O arquivo de perguntas ('{perguntas_csv}') está vazio."); st.stop()
                        ...
                        for _, p_row_prog_novo in perguntas_df_formulario.iterrows():
                            ...
                            if resp_prog_novo is not None:
                                ...
                                if "[Matriz GUT]" in p_texto_prog_novo:
                                    if isinstance(resp_prog_novo, dict) and (int(resp_prog_novo.get("G",0)) > 0 or int(resp_prog_novo.get("U",0)) > 0 or int(resp_prog_novo.get("T",0)) > 0):
                                        is_answered = True
                                elif "Escala" in p_texto_prog_novo: # e.g. "Pergunta [Escala]"
                                    if resp_prog_novo != "Selecione":
                                        is_answered = True
                                elif isinstance(resp_prog_novo, str): # For "Texto Aberto"
                                    if resp_prog_novo.strip() : is_answered = True
                                elif isinstance(resp_prog_novo, (int,float)): # For "Pontuação"
                                     # Assuming 0 can be a valid deliberate answer for pontuacao, but for progress count, any explicit set value counts.
                                     # Let's refine: if it's a slider, it always has a value. How to define "answered"?
                                     # For sliders (0-5, 0-10), perhaps any interaction counts.
                                     # The current logic counts if it's not None. For sliders, initial 0 might be "unanswered".
                                     # Let's consider a non-default value or any explicit interaction.
                                     # For now, if not None, it's counted. This might need refinement based on desired behavior for 0.
                                     is_answered = True # Defaulting to count if not None.
                                if is_answered: respondidas_novo +=1
                    ...
                    def on_change_resposta_novo(pergunta_txt_key_novo, widget_st_key_novo, tipo_pergunta_onchange_novo):
                        ...
                        if tipo_pergunta_onchange_novo.startswith("GUT_"):
                            current_gut_novo = st.session_state.respostas_atuais_diagnostico.get(pergunta_txt_key_novo, {"G":0,"U":0,"T":0}).copy() # ensure copy
                            if tipo_pergunta_onchange_novo == "GUT_G": current_gut_novo["G"] = int(valor_widget_novo)
                            elif tipo_pergunta_onchange_novo == "GUT_U": current_gut_novo["U"] = int(valor_widget_novo)
                            elif tipo_pergunta_onchange_novo == "GUT_T": current_gut_novo["T"] = int(valor_widget_novo)
                        ...
                        calcular_e_mostrar_progresso_novo() # Recalculate progress on each change
                    ...
                    for categoria_novo in sorted(perguntas_df_formulario["Categoria"].unique()):
                        ...
                        for idx_novo, row_q_novo in perg_cat_df_novo.iterrows():
                            ...
                            with cols_q_feedback[0]:
                                default_val = st.session_state.respostas_atuais_diagnostico.get(p_texto_novo)
                                if "[Matriz GUT]" in p_texto_novo:
                                    st.markdown(f"**{p_texto_novo.replace(' [Matriz GUT]', '')}**"); cols_gut_w_novo = st.columns(3)
                                    gut_vals_novo = default_val if isinstance(default_val, dict) else {"G":0,"U":0,"T":0}
                                ...
                                elif "Pontuação (0-5)" in p_texto_novo: st.slider(p_texto_novo,0,5,value=int(default_val if default_val is not None else 0), key=w_key_novo, on_change=on_change_resposta_novo, args=(p_texto_novo, w_key_novo, "Slider05"))
                                elif "Pontuação (0-10)" in p_texto_novo: st.slider(p_texto_novo,0,10,value=int(default_val if default_val is not None else 0), key=w_key_novo, on_change=on_change_resposta_novo, args=(p_texto_novo, w_key_novo, "Slider010"))
                                elif "Texto Aberto" in p_texto_novo: st.text_area(p_texto_novo,value=str(default_val if default_val is not None else ""), key=w_key_novo, on_change=on_change_resposta_novo, args=(p_texto_novo, w_key_novo, "Texto"))
                                elif "Escala" in p_texto_novo:
                                    opts_novo = ["Selecione", "Muito Baixo", "Baixo", "Médio", "Alto", "Muito Alto"]
                                    curr_val_novo = default_val if default_val in opts_novo else "Selecione"
                                ...
                                else: # Default to slider 0-10 if type not recognized but not GUT
                                    st.slider(p_texto_novo,0,10,value=int(default_val if default_val is not None else 0), key=w_key_novo, on_change=on_change_resposta_novo, args=(p_texto_novo, w_key_novo, "SliderDefault"))
                    ...
                    if st.button("✔️ Concluir e Enviar Diagnóstico", key="enviar_diag_final_cliente_v16_nd"):
                        ...
                        # Re-calculate progress one last time before submission check
                        calcular_e_mostrar_progresso_novo()
                        cont_resp_n, total_para_resp_n = st.session_state.progresso_diagnostico_contagem # get updated values
                        ...
                        if cont_resp_n < total_para_resp_n: st.warning("Por favor, responda todas as perguntas para um diagnóstico completo.")
                            ...
                            for p_n,r_n in respostas_finais_envio_novo.items():
                                ...
                                if "[Matriz GUT]" in p_n and isinstance(r_n, dict): 
                                    ...
                                    g_n,u_n,t_n = int(r_n.get("G",0)), int(r_n.get("U",0)), int(r_n.get("T",0))
                                    soma_gut_n += (g_n*u_n*t_n); count_gut_n +=1
                                else: respostas_csv_n[p_n] = r_n
                            
                            gut_media_n = round(soma_gut_n/count_gut_n,2) if count_gut_n > 0 else 0.0
                            num_resp_n = [pd.to_numeric(v_n, errors='coerce') for k_n,v_n in respostas_finais_envio_novo.items() 
                                          if not k_n.startswith("__") and 
                                          (isinstance(v_n,(int,float)) or (isinstance(v_n, str) and v_n.replace('.','',1).isdigit())) and
                                          ("[Matriz GUT]" not in k_n) and 
                                          ("Pontuação" in k_n or "Escala" not in k_n) # Count pontuacao, not raw escala text
                                         ]
                            num_resp_n = [x for x in num_resp_n if pd.notna(x)] # Filter out NaNs from coerced strings
                            ...
                            nova_linha_diag_final_n = { 
                                ...
                                "Email": "", # Placeholder, add if needed
                                "Empresa": emp_nome_n, 
                                "Média Geral": media_geral_n, 
                                "GUT Média": gut_media_n, 
                                "Observações": "", # Placeholder
                                "Análise do Cliente": respostas_finais_envio_novo.get("__obs_cliente__",""), 
                                "Diagnóstico": respostas_finais_envio_novo.get("__resumo_cliente__",""), 
                                "Comentarios_Admin": "" 
                            }
                            ...
                            for cat_iter_n in perguntas_df_formulario["Categoria"].unique():
                                ...
                                for _,pr_n in perguntas_df_formulario[perguntas_df_formulario["Categoria"]==cat_iter_n].iterrows():
                                    ...
                                    rv_n_numeric = pd.to_numeric(rv_n, errors='coerce')
                                    if pd.notna(rv_n_numeric) and ("[Matriz GUT]" not in pt_n) and ("Pontuação" in pt_n or "Escala" not in pt_n): # Count pontuacao, not raw escala text
                                        soma_c_n+=rv_n_numeric; cont_c_n+=1
                            ...
                            for col_n_n in nova_linha_diag_final_n.keys():
                                if col_n_n not in df_todos_diags_n.columns: df_todos_diags_n[col_n_n] = pd.NA # Add missing columns
                            ...
                            # Also reset instruction confirmation for this slot as it's now used
                            ...
                            if st.session_state.user: 
                                ...
                                st.session_state.user["ConfirmouInstrucoesParaSlotAtual"] = False # Update session state too
                            ...
                            # Reset form state AFTER successful submission and PDF generation attempt
                            st.session_state.respostas_atuais_diagnostico = {}
                            st.session_state.progresso_diagnostico_percentual = 0
                            st.session_state.progresso_diagnostico_contagem = (0,total_perguntas_form if total_perguntas_form else 0)
                            st.session_state.feedbacks_respostas = {}
                            st.rerun() # Rerun to show success message and download on the "Novo Diagnóstico" page or redirect
                    ...
                    try:
                        ...
                        if not df_notif_cliente_view.empty: 
                            df_notif_cliente_view['DataHora'] = pd.to_datetime(df_notif_cliente_view['DataHora'])
                            df_notif_cliente_view = df_notif_cliente_view.sort_values(by="DataHora", ascending=False)
                    ...
                    if df_notif_cliente_view.empty: st.info("Você não tem nenhuma notificação no momento.")
                        ...
                        if ids_nao_lidas_para_marcar_view: # Only mark if there are unread ones
                            if 'notif_page_loaded_once_v14_final_c' not in st.session_state: # Mark only once per page load sequence
                                if marcar_notificacoes_como_lidas(st.session_state.cnpj, ids_notificacoes=ids_nao_lidas_para_marcar_view):
                                    st.session_state.notif_page_loaded_once_v14_final_c = True # Set flag
                                    st.rerun() # Rerun to update notification count in sidebar
                        elif 'notif_page_loaded_once_v14_final_c' in st.session_state: # If all are read, clear the flag
                            del st.session_state.notif_page_loaded_once_v14_final_c
                # --- End of original "CLIENTE LOGADO" area ---
            except IndexError:
                st.error("Não foi possível carregar os dados do cliente selecionado. Verifique o arquivo de usuários.")
                reset_client_session_state()
            except Exception as e_client_view:
                st.error(f"Ocorreu um erro na visualização do cliente: {e_client_view}")
                st.exception(e_client_view)
                # reset_client_session_state() # Optionally reset
        else: # No client selected yet
            st.info("Por favor, selecione um cliente na barra lateral para visualizar seus diagnósticos e opções.")
            # Optionally clear any stale client data from session state if no client is selected
            if st.session_state.cnpj is not None: # if a client was previously selected
                 st.session_state.cnpj = None
                 st.session_state.user = None
                 st.session_state.cliente_page = "Instruções" # Reset to a safe default
                 # Add other necessary resets if needed

    except FileNotFoundError:
        st.sidebar.error(f"Arquivo de usuários '{usuarios_csv}' não encontrado. A seleção de cliente está desabilitada.")
        st.error(f"Arquivo crucial '{usuarios_csv}' não encontrado. Funcionalidades limitadas.")
    except Exception as e_load_users_select:
        st.sidebar.error("Erro ao carregar lista de clientes.")
        st.error(f"Erro ao carregar clientes para seleção: {e_load_users_select}")


elif st.session_state.current_app_mode == "Painel Administrativo":
    # --- This is the start of the original "ADMINISTRADOR LOGADO" area ---
    st.session_state.admin_logado_for_internal_checks = True # For internal checks if any part of admin code uses this
    try:
        try: st.sidebar.image("https://via.placeholder.com/150x75.png?text=Sua+Logo+Admin", width=150) # Placeholder
        ...
        # The "Sair do Painel Admin" can switch mode or just be visual
        if st.sidebar.button("↩️ Voltar à Seleção de Modo", key="admin_go_back_mode_v16"):
            st.session_state.current_app_mode = "Visualização Cliente"
            st.session_state.selected_client_identifier_for_view = None # Reset client selection
            st.rerun()
        ...
        menu_admin = st.sidebar.selectbox("Funcionalidades Admin:", menu_admin_options, key="admin_menu_selectbox_v16_adm")
        ...
        try:
            ...
            for col, default, dtype_col in [("ConfirmouInstrucoesParaSlotAtual", "False", str), 
        ...
        elif os.path.getsize(arquivo_csv) == 0: st.warning(f"O arquivo de diagnósticos '{arquivo_csv}' está vazio (0 bytes).")
        else:
            try:
                ...
                else: # File has content (e.g. headers) but pandas read it as empty
                    st.warning(f"Arquivo de diagnósticos '{arquivo_csv}' lido como vazio (pode conter apenas cabeçalhos).")

            except pd.errors.EmptyDataError: st.warning(f"Arquivo de diagnósticos '{arquivo_csv}' vazio ou apenas cabeçalhos.")
        ...
        # --- Admin Menu Dispatch Logic (original logic mostly) ---
        try:
            if menu_admin == "📊 Visão Geral e Diagnósticos":
                ...
                with col_f1_vg: emp_sel_admin_vg = st.selectbox("Filtrar por Empresa:", ["Todos os Clientes"] + empresas_lista_admin_filtro_vg, key="admin_filtro_emp_v16_vg")
                with col_f2_vg: dt_ini_admin_vg = st.date_input("Data Início dos Diagnósticos:", value=None, key="admin_dt_ini_v16_vg")
                with col_f3_vg: dt_fim_admin_vg = st.date_input("Data Fim dos Diagnósticos:", value=None, key="admin_dt_fim_v16_vg")
                st.divider()
                ...
                if 'Data' in df_diagnosticos_filtrados_view_final_vg.columns: # Ensure 'Data' column exists before filtering
                    if dt_ini_admin_vg: df_diagnosticos_filtrados_view_final_vg = df_diagnosticos_filtrados_view_final_vg[df_diagnosticos_filtrados_view_final_vg['Data'].dt.date >= dt_ini_admin_vg]
                    if dt_fim_admin_vg: df_diagnosticos_filtrados_view_final_vg = df_diagnosticos_filtrados_view_final_vg[df_diagnosticos_filtrados_view_final_vg['Data'].dt.date <= dt_fim_admin_vg]
                ...
                if not admin_data_carregada_view_sucesso and os.path.exists(arquivo_csv): # File exists but no data loaded
                    st.warning("Nenhum dado de diagnóstico foi carregado (o arquivo pode estar vazio de registros válidos ou conter apenas cabeçalhos). Funcionalidades de visualização detalhada estão limitadas.")
                elif df_diagnosticos_filtrados_view_final_vg.empty: 
                    ...
                    if not df_diagnosticos_filtrados_view_final_vg.empty:
                        diagnosticos_para_detalhe_admin = df_diagnosticos_filtrados_view_final_vg.apply(
                            lambda row: f"{pd.to_datetime(row['Data']).strftime('%Y-%m-%d %H:%M')} - {row['Empresa']} (Índice Original: {row.name})", axis=1).tolist()
                        diag_selecionado_str_admin = st.selectbox("Selecione um Diagnóstico para Detalhar:", [""] + diagnosticos_para_detalhe_admin, key="admin_select_diag_detalhe_v16")
                        if diag_selecionado_str_admin:
                            try:
                                diag_original_index_admin = int(diag_selecionado_str_admin.split("(Índice Original: ")[1].replace(")", ""))
                                diag_row_detalhe_admin_series = diagnosticos_df_admin_orig_view.loc[diag_original_index_admin]
                                diag_row_detalhe_admin = diag_row_detalhe_admin_series.to_dict()

                                st.markdown(f"###### Detalhes do Diagnóstico: {diag_row_detalhe_admin.get('Data')} - {diag_row_detalhe_admin.get('Empresa')}")
                                comentarios_admin_atuais_det = diag_row_detalhe_admin.get('Comentarios_Admin', "")
                                novos_comentarios_admin_det = st.text_area("Comentários do Consultor:", value=comentarios_admin_atuais_det, key=f"com_admin_det_{diag_original_index_admin}")
                                if st.button("Salvar Comentários do Consultor 💬", key=f"save_com_admin_det_{diag_original_index_admin}"):
                                    ...
                                    df_all_diags_update.loc[diag_original_index_admin, 'Comentarios_Admin'] = novos_comentarios_admin_det
                                    df_all_diags_update.to_csv(arquivo_csv, index=False, encoding='utf-8')
                                    criar_notificacao(diag_row_detalhe_admin.get('CNPJ'), "Novos comentários do consultor disponíveis.", str(diag_row_detalhe_admin.get('Data'))); 
                                    st.success("Comentários salvos e cliente notificado!"); st.rerun()
                                if st.button("📄 Baixar PDF deste Diagnóstico", key=f"dl_pdf_admin_detalhe_v16_{diag_original_index_admin}"):
                                    try: usuario_do_diag_pdf_adm = df_usuarios_admin_geral[df_usuarios_admin_geral['CNPJ'] == diag_row_detalhe_admin.get('CNPJ')].iloc[0].to_dict()
                                    except: usuario_do_diag_pdf_adm = {"Empresa": diag_row_detalhe_admin.get("Empresa","N/A"), "CNPJ": diag_row_detalhe_admin.get("CNPJ","N/A")}
                                    
                                    perguntas_df_pdf_admin_det = pd.read_csv(perguntas_csv, encoding='utf-8')
                                    analises_df_pdf_admin_det = carregar_analises_perguntas()
                                    medias_cat_pdf_admin_det = {k.replace("Media_Cat_","").replace("_"," "):v for k,v in diag_row_detalhe_admin.items() if isinstance(k,str) and "Media_Cat_" in k and pd.notna(v)}
                                    
                                    pdf_path_admin_det = gerar_pdf_diagnostico_completo(diag_row_detalhe_admin, usuario_do_diag_pdf_adm, perguntas_df_pdf_admin_det, diag_row_detalhe_admin, medias_cat_pdf_admin_det, analises_df_pdf_admin_det)
                                    if pdf_path_admin_det:
                                        with open(pdf_path_admin_det, "rb") as f_pdf_admin_det: 
                                            st.download_button("Download PDF Confirmado", f_pdf_admin_det, file_name=f"diagnostico_admin_{sanitize_column_name(diag_row_detalhe_admin.get('Empresa'))}_{str(diag_row_detalhe_admin.get('Data')).replace(':','-').replace(' ','_')}.pdf", mime="application/pdf", key=f"dl_conf_admin_detalhe_v16_{diag_original_index_admin}")
                                    else: st.error("Falha ao gerar PDF.")
                            except (IndexError, KeyError, ValueError) as e_lookup: st.warning(f"Não foi possível carregar os detalhes do diagnóstico. Erro: {e_lookup}")
                            except Exception as e_detalhe: st.error(f"Erro ao detalhar diagnóstico: {e_detalhe}")
                ...
                df_diagnosticos_status_geral = diagnosticos_df_admin_orig_view.copy() if admin_data_carregada_view_sucesso else pd.DataFrame()
                ...
                emp_sel_status_view = st.selectbox("Filtrar por Empresa:", empresas_status_list_view, key="status_emp_sel_v16")
                ...
                if emp_sel_status_view != "Todas":
                    ...
                    if not df_diagnosticos_status_filtrado_local.empty: 
                        df_diagnosticos_status_filtrado_local = df_diagnosticos_status_filtrado_local[df_diagnosticos_status_filtrado_local["Empresa"] == emp_sel_status_view]
                
                if df_usuarios_status_filtrado.empty: st.info(f"Nenhum cliente encontrado para a empresa '{emp_sel_status_view}'.")
                else:
                    st.markdown("##### Clientes que JÁ REALIZARAM pelo menos um diagnóstico:")
                    if not df_diagnosticos_status_filtrado_local.empty:
                        ...
                        else: st.info(f"Nenhum cliente da empresa '{emp_sel_status_view}' realizou diagnósticos (ou não há diagnósticos registrados para esta empresa).")
                    elif admin_data_carregada_view_sucesso: st.info(f"Nenhum diagnóstico registrado no sistema para a empresa '{emp_sel_status_view}'.")
                    else: st.info("Dados de diagnóstico não disponíveis (arquivo pode estar ausente/vazio).")
                    
                    st.markdown("---")
                    st.markdown("##### Clientes com Diagnósticos LIBERADOS e AINDA NÃO REALIZADOS (ou com slots pendentes):")
                    clientes_liberados_pendentes_status = df_usuarios_status_filtrado[
                        df_usuarios_status_filtrado['DiagnosticosDisponiveis'] > df_usuarios_status_filtrado['TotalDiagnosticosRealizados']
                    ]
                ...
                emp_sel_hu = col_hu_f1.selectbox("Filtrar por Empresa:", empresas_hist_list_hu, key="hist_emp_sel_v16_hu_adm")
                termo_busca_hu = col_hu_f2.text_input("Buscar por Nome, CNPJ, Ação ou Descrição:", key="hist_termo_busca_v16_hu_adm")
                ...
                if not df_historico_filtrado_view_hu.empty:
                    ...
                    if st.button("📄 Baixar Histórico Filtrado (PDF)", key="download_hist_filtrado_pdf_v16_hu_adm"):
                        titulo_pdf_hist_file = f"Historico_Acoes_{sanitize_column_name(emp_sel_hu)}_{sanitize_column_name(termo_busca_hu) if termo_busca_hu else 'Todos'}_{datetime.now().strftime('%Y%m%d')}.pdf"
                        ...
                        if pdf_path_hist:
                            with open(pdf_path_hist, "rb") as f_pdf_hist: 
                                st.download_button(label="Download Confirmado", data=f_pdf_hist, file_name=titulo_pdf_hist_file, mime="application/pdf", key="confirm_download_hist_pdf_v16_hu_adm")
                        else: st.error("Falha ao gerar PDF do histórico.")
                else: st.info("Nenhum registro de histórico encontrado para os filtros aplicados.")

            # ... (Rest of Admin menu items: Gerenciar Perguntas, Análises, Instruções, Clientes, Admins, Backup)
            # These sections are extensive but generally don't depend on the login state in the same way,
            # they operate on the CSV files directly. The FPDF fix was the main bug there.
            # For brevity, I'm assuming these sections will work once the FPDF and file loading is stable.
            # I will include them as they are, with minor key changes if necessary for uniqueness.
                ...
                with tabs_perg_admin[0]:
                    if perguntas_df_admin_gp.empty: st.info("Nenhuma pergunta cadastrada.")
                        ...
                        for i_p_admin, row_p_admin in perguntas_df_admin_gp.iterrows():
                            ...
                            with cols_p_admin[0]: nova_p_text_admin = st.text_input("Pergunta", value=str(row_p_admin["Pergunta"]), key=f"edit_p_txt_v16_gp_{i_p_admin}")
                            with cols_p_admin[1]: nova_cat_text_admin = st.text_input("Categoria", value=str(row_p_admin.get("Categoria", "Geral")), key=f"edit_p_cat_v16_gp_{i_p_admin}")
                            with cols_p_admin[2]:
                                st.write("") # For alignment
                                if st.button("💾", key=f"salvar_p_adm_v16_gp_{i_p_admin}", help="Salvar"):
                            ...
                            with cols_p_admin[3]:
                                st.write("") # For alignment
                                if st.button("🗑️", key=f"deletar_p_adm_v16_gp_{i_p_admin}", help="Deletar"):
                ...
                with tabs_perg_admin[1]:
                    with st.form("form_nova_pergunta_admin_v16_gp"):
                        st.subheader("➕ Adicionar Nova Pergunta"); nova_p_form_txt_admin = st.text_input("Texto da Pergunta", key="nova_p_input_admin_txt_v16_gp")
                        cat_existentes_gp = sorted(list(perguntas_df_admin_gp['Categoria'].astype(str).unique())) if not perguntas_df_admin_gp.empty and "Categoria" in perguntas_df_admin_gp.columns else []; cat_options_gp = ["Nova Categoria"] + cat_existentes_gp; cat_selecionada_gp = st.selectbox("Categoria:", cat_options_gp, key="cat_select_admin_new_q_v16_gp")
                        nova_cat_form_admin_gp = st.text_input("Nome da Nova Categoria:", key="nova_cat_input_admin_new_q_v16_gp") if cat_selecionada_gp == "Nova Categoria" else cat_selecionada_gp
                        tipo_p_form_admin = st.selectbox("Tipo de Pergunta (será adicionado ao final do texto da pergunta):", ["Pontuação (0-10)", "Pontuação (0-5)", "Texto Aberto", "Escala (Muito Baixo, Baixo, Médio, Alto, Muito Alto)", "[Matriz GUT]"], key="tipo_p_select_admin_new_q_v16_gp")
                        if st.form_submit_button("➕ Adicionar Pergunta"):
                            if nova_p_form_txt_admin.strip() and nova_cat_form_admin_gp.strip(): 
                                ...
                                perguntas_df_admin_gp.to_csv(perguntas_csv, index=False, encoding='utf-8'); st.success(f"Pergunta adicionada!"); st.rerun()
                ...
                if df_perguntas_formulario_admin.empty: st.warning("Nenhuma pergunta cadastrada. Adicione perguntas primeiro.")
                else:
                    lista_perguntas_txt_admin = [""] + df_perguntas_formulario_admin["Pergunta"].unique().tolist(); pergunta_selecionada_analise_admin = st.selectbox("Selecione a Pergunta para adicionar análise:", lista_perguntas_txt_admin, key="sel_perg_analise_v16_ga")
                    if pergunta_selecionada_analise_admin:
                        st.caption(f"Pergunta selecionada: {pergunta_selecionada_analise_admin}"); tipo_condicao_analise_display_admin = st.selectbox("Tipo de Condição para a Análise:", ["Faixa Numérica (p/ Pontuação 0-X)", "Valor Exato (p/ Escala)", "Faixa de Score (p/ Matriz GUT)", "Análise Padrão (default para a pergunta)"], key="tipo_cond_analise_v16_ga")
                        ...
                        elif tipo_condicao_csv_val_admin == "ValorExatoEscala": cond_val_exato_ui_admin = st.text_input("Valor Exato da Escala (ex: Baixo, Médio, Alto)", key="cond_exato_analise_v16_ga")
                        elif tipo_condicao_csv_val_admin == "ScoreGUT": cols_faixa_gut_ui_admin = st.columns(2); cond_val_min_ui_admin = cols_faixa_gut_ui_admin[0].number_input("Score GUT Mínimo", step=1, key="cond_min_gut_analise_v16_ga"); cond_val_max_ui_admin = cols_faixa_gut_ui_admin[1].number_input("Score GUT Máximo (opcional, deixe 0 ou vazio se for 'acima de Mínimo')", value=0.0, step=1.0, format="%.0f", key="cond_max_gut_analise_v16_ga")
                        texto_analise_nova_ui_admin = st.text_area("Texto da Análise:", height=150, key="txt_analise_nova_v16_ga")
                        if st.button("💾 Salvar Nova Análise", key="salvar_analise_pergunta_v16_ga"):
                ...
                df_analises_para_exibir = carregar_analises_perguntas() # Re-load
                if df_analises_para_exibir.empty: st.info("Nenhuma análise cadastrada.")
                    ...
                    analise_del_id_admin_view = st.selectbox("Deletar Análise por ID:", [""] + df_analises_para_exibir["ID_Analise"].astype(str).tolist(), key="del_analise_id_v16_ga_view")
                    if st.button("🗑️ Deletar Análise", key="btn_del_analise_v16_ga_view") and analise_del_id_admin_view: df_analises_para_exibir = df_analises_para_exibir[df_analises_para_exibir["ID_Analise"] != analise_del_id_admin_view]; df_analises_para_exibir.to_csv(analises_perguntas_csv, index=False, encoding='utf-8'); st.warning("Análise deletada."); st.rerun()
                ...
                try:
                    if os.path.exists(instrucoes_txt_file) and os.path.getsize(instrucoes_txt_file) > 0:
                        with open(instrucoes_txt_file, "r", encoding="utf-8") as f: current_instructions = f.read()
                    elif not os.path.exists(instrucoes_txt_file): 
                        with open(instrucoes_txt_file, "w", encoding="utf-8") as f: default_text = "**Bem-vindo!** (Instruções padrão)"; f.write(default_text); current_instructions = default_text
                except Exception as e: st.error(f"Erro ao ler/criar arquivo de instruções: {e}")
                edited_instructions = st.text_area("Edite as instruções (Markdown):", value=current_instructions, height=400, key="admin_edit_instructions_ta_v16")
                if st.button("💾 Salvar Instruções", key="admin_save_instructions_btn_v16"):
                    try:
                        with open(instrucoes_txt_file, "w", encoding="utf-8") as f: f.write(edited_instructions)
                        st.success("Instruções salvas!"); st.rerun()
                ...
                except FileNotFoundError: st.error(f"Arquivo '{usuarios_csv}' não encontrado."); df_usuarios_gc = pd.DataFrame(columns=colunas_base_usuarios)
                ...
                if not df_usuarios_gc.empty:
                    cols_display_gc = ["CNPJ", "Empresa", "NomeContato", "Telefone", "DiagnosticosDisponiveis", "TotalDiagnosticosRealizados", "LiberacoesExtrasConcedidas", "ConfirmouInstrucoesParaSlotAtual"]
                    st.dataframe(df_usuarios_gc[cols_display_gc])
                    st.markdown("#### Ações de Cliente"); clientes_lista_gc_ops = df_usuarios_gc.apply(lambda row: f"{row['Empresa']} ({row['CNPJ']})", axis=1).tolist(); cliente_selecionado_str_gc = st.selectbox("Selecione o cliente para gerenciar:", [""] + clientes_lista_gc_ops, key="sel_cliente_gc_v16_adm")
                    if cliente_selecionado_str_gc:
                        ...
                        st.write(f"**Empresa:** {cliente_data_gc_val['Empresa']}"); st.write(f"**Slots Disponíveis:** {cliente_data_gc_val['DiagnosticosDisponiveis']}"); st.write(f"**Realizados:** {cliente_data_gc_val['TotalDiagnosticosRealizados']}"); st.write(f"**Extras Concedidos:** {cliente_data_gc_val['LiberacoesExtrasConcedidas']}")
                        if st.button(f"➕ Conceder +1 Diagnóstico para {cliente_data_gc_val['Empresa']}", key=f"conceder_diag_gc_v16_adm_{cnpj_selecionado_gc_val}"):
                            ...
                            update_user_data(cnpj_selecionado_gc_val, "DiagnosticosDisponiveis", novos_disponiveis); update_user_data(cnpj_selecionado_gc_val, "LiberacoesExtrasConcedidas", liberacoes_extras_atuais + 1); update_user_data(cnpj_selecionado_gc_val, "ConfirmouInstrucoesParaSlotAtual", "False") # Reset confirmation for new slot
                            registrar_acao("ADMIN", "Concessão Diagnóstico", f"Admin concedeu +1 slot para {cliente_data_gc_val['Empresa']} ({cnpj_selecionado_gc_val}). Slots: {novos_disponiveis}. Extras: {liberacoes_extras_atuais + 1}."); st.success(f"+1 Slot concedido."); st.rerun()
                        ...
                        if is_blocked_gc_check:
                            if st.button(f"🔓 Desbloquear Acesso para {cliente_data_gc_val['Empresa']}", key=f"desbloq_total_gc_v16_adm_{cnpj_selecionado_gc_val}"): bloqueados_df_gc_check = bloqueados_df_gc_check[bloqueados_df_gc_check["CNPJ"] != cnpj_selecionado_gc_val]; bloqueados_df_gc_check.to_csv(usuarios_bloqueados_csv, index=False, encoding='utf-8'); st.success(f"Acesso desbloqueado."); st.rerun()
                        else:
                            if st.button(f"🚫 Bloquear Acesso para {cliente_data_gc_val['Empresa']}", type="primary", key=f"bloq_total_gc_v16_adm_{cnpj_selecionado_gc_val}"): nova_entrada_bloqueio_gc_val = pd.DataFrame([{"CNPJ": cnpj_selecionado_gc_val}]); bloqueados_df_gc_check = pd.concat([bloqueados_df_gc_check, nova_entrada_bloqueio_gc_val], ignore_index=True); bloqueados_df_gc_check.to_csv(usuarios_bloqueados_csv, index=False, encoding='utf-8'); st.error(f"Acesso bloqueado."); st.rerun()
                else: st.info("Nenhum cliente cadastrado.")
                st.markdown("---"); st.markdown("#### Adicionar Novo Cliente")
                with st.form("form_novo_cliente_v16_adm", clear_on_submit=True):
                    novo_cnpj_gc_form = st.text_input("CNPJ do Novo Cliente:"); nova_senha_gc_form = st.text_input("Senha para o Cliente:", type="password"); nova_empresa_gc_form = st.text_input("Nome da Empresa:"); novo_contato_gc_form = st.text_input("Nome do Contato (opcional):"); novo_telefone_gc_form = st.text_input("Telefone (opcional):")
                    if st.form_submit_button("➕ Cadastrar Novo Cliente"):
                        if novo_cnpj_gc_form and nova_senha_gc_form and nova_empresa_gc_form:
                            if df_usuarios_gc.empty or (novo_cnpj_gc_form not in df_usuarios_gc["CNPJ"].values): 
                                ...
                                df_usuarios_gc_updated.to_csv(usuarios_csv, index=False, encoding='utf-8'); st.success(f"Cliente {nova_empresa_gc_form} cadastrado!"); st.rerun()
                ...
                with st.form("form_novo_admin_mng_v16_adm"):
                ...
                if not admins_df_mng.empty:
                    admin_para_remover_mng = st.selectbox("Remover Admin:", options=[""] + admins_df_mng["Usuario"].tolist(), key="remove_admin_select_mng_v16_adm")
                    if st.button("Remover Admin Selecionado", type="primary", key="btn_remove_admin_v16_adm") and admin_para_remover_mng:
                ...
                st.subheader("Backup de Dados do Sistema"); st.markdown("Clique nos botões abaixo para baixar cópias dos arquivos CSV.")
                ...
                for nome_amigavel, nome_arquivo in arquivos_para_backup.items():
                    if os.path.exists(nome_arquivo) and os.path.getsize(nome_arquivo) > 0:
                        with open(nome_arquivo, "rb") as fp: st.download_button(label=f"Baixar {nome_amigavel} ({os.path.getsize(nome_arquivo)} bytes)", data=fp, file_name=nome_arquivo, mime="text/csv", key=f"backup_btn_v16_{nome_arquivo.replace('.','_')}")
                    elif os.path.exists(nome_arquivo): st.warning(f"Arquivo '{nome_arquivo}' encontrado, mas está vazio.")
                    else: st.warning(f"Arquivo '{nome_arquivo}' não encontrado para backup.")
        
        except Exception as e_admin_menu_dispatch:
            st.error(f"Ocorreu um erro na funcionalidade Admin '{menu_admin}': {e_admin_menu_dispatch}")
...
# Fallback if no mode is somehow active, though sidebar radio should prevent this.
if 'current_app_mode' not in st.session_state :
     st.info("Selecione um modo de operação na barra lateral.")