...
def find_client_logo_path(cnpj):
    ...
    base_name = str(cnpj).replace('/', '').replace('.', '').replace('-', '') # Limpa CNPJ para nome de arquivo
...
if not os.path.exists(LOGOS_DIR):
    ...
    except OSError as e_logo_dir_create:
        st.error(f"Não foi possível criar o diretório de logos '{LOGOS_DIR}': {e_logo_dir_create}")
...
def inicializar_csv(filepath, columns, is_perguntas_file=False):
    ...
    except Exception as e_init_csv_func:
        st.error(f"Erro ao inicializar/verificar {filepath}: {e_init_csv_func}. O app pode não funcionar corretamente.")
        st.exception(e_init_csv_func) # Mostrar traceback para depuração
...
except Exception as e_init_all_scripts:
    st.error(f"Falha crítica na inicialização de arquivos CSV base: {e_init_all_scripts}")
    st.exception(e_init_all_scripts)
    st.markdown("Verifique permissões de arquivo e a integridade dos CSVs existentes. Delete-os para recriação se necessário.")
    st.stop()
...
def gerar_pdf_diagnostico_completo(diagnostico_data, usuario_data, perguntas_df_geracao, respostas_coletadas_geracao, medias_categorias_geracao):
    try:
        ...
        logo_path_pdf_gen = find_client_logo_path(cnpj_pdf)
        if logo_path_pdf_gen:
            try: 
                current_y = pdf.get_y()
                pdf.image(logo_path_pdf_gen, x=10, y=current_y, w=33, h=15) # Ajuste h se necessário
                pdf.set_y(current_y + 15 + 5) # Pular linha após a logo (altura da logo + margem)
            except Exception: pass # Ignora erro de logo no PDF, continua sem
        ...
        for titulo_pdf, campo_dado_pdf in [("Resumo do Diagnóstico (Cliente):", "Diagnóstico"), 
            ...
            if valor_campo_pdf and not pd.isna(valor_campo_pdf): # Checa se não é NaN antes de usar
        ...
        categorias_unicas_pdf_gen = []
        if perguntas_df_geracao is not None and "Categoria" in perguntas_df_geracao.columns: 
            categorias_unicas_pdf_gen = perguntas_df_geracao["Categoria"].unique()
        
        for categoria_pdf_det_gen in categorias_unicas_pdf_gen:
            pdf.set_font("Arial", 'B', 10); pdf.multi_cell(0, 7, pdf_safe_text_output(f"Categoria: {categoria_pdf_det_gen}"))
            pdf.set_font("Arial", size=9)
            perguntas_cat_pdf_det_gen = perguntas_df_geracao[perguntas_df_geracao["Categoria"] == categoria_pdf_det_gen]
            for _, p_row_pdf_det_gen in perguntas_cat_pdf_det_gen.iterrows():
                txt_p_pdf_det_gen = p_row_pdf_det_gen["Pergunta"]
                resp_p_pdf_det_gen = respostas_coletadas_geracao.get(txt_p_pdf_det_gen) 
                if resp_p_pdf_det_gen is None: 
                    resp_p_pdf_det_gen = diagnostico_data.get(txt_p_pdf_det_gen, "N/R")

                if isinstance(txt_p_pdf_det_gen, str) and "[Matriz GUT]" in txt_p_pdf_det_gen: 
                    g_pdf_val, u_pdf_val, t_pdf_val = 0,0,0
                    score_gut_item_pdf_val = 0
                    if isinstance(resp_p_pdf_det_gen, dict): 
                        g_pdf_val,u_pdf_val,t_pdf_val = resp_p_pdf_det_gen.get("G",0), resp_p_pdf_det_gen.get("U",0), resp_p_pdf_det_gen.get("T",0)
                    elif isinstance(resp_p_pdf_det_gen, str): 
                        try: 
                            gut_data_pdf_val = json.loads(resp_p_pdf_det_gen.replace("'", "\""))
                            g_pdf_val,u_pdf_val,t_pdf_val = gut_data_pdf_val.get("G",0), gut_data_pdf_val.get("U",0), gut_data_pdf_val.get("T",0)
                        except: pass 
                    score_gut_item_pdf_val = g_pdf_val*u_pdf_val*t_pdf_val
                    pdf.multi_cell(0,6,pdf_safe_text_output(f"  - {txt_p_pdf_det_gen.replace(' [Matriz GUT]','')}: G={g_pdf_val}, U={u_pdf_val}, T={t_pdf_val} (Score: {score_gut_item_pdf_val})"))
                elif isinstance(resp_p_pdf_det_gen, (int, float, str)): 
                    pdf.multi_cell(0, 6, pdf_safe_text_output(f"  - {txt_p_pdf_det_gen}: {resp_p_pdf_det_gen}"))
            pdf.ln(2)
        pdf.ln(3)
        ...
        pdf.set_font("Arial", size=10)
        gut_cards_pdf_gen_list = [] 
        for pergunta_pdf_k_gen, resp_pdf_k_val_gen in respostas_coletadas_geracao.items(): 
            if isinstance(pergunta_pdf_k_gen, str) and "[Matriz GUT]" in pergunta_pdf_k_gen:
                g_k_gen, u_k_gen, t_k_gen = 0,0,0
                if isinstance(resp_pdf_k_val_gen, dict):
                    g_k_gen, u_k_gen, t_k_gen = resp_pdf_k_val_gen.get("G",0), resp_pdf_k_val_gen.get("U",0), resp_pdf_k_val_gen.get("T",0)
                elif isinstance(resp_pdf_k_val_gen, str): 
                    try: 
                        gut_data_k_gen = json.loads(resp_pdf_k_val_gen.replace("'", "\""))
                        g_k_gen,u_k_gen,t_k_gen = gut_data_k_gen.get("G",0), gut_data_k_gen.get("U",0), gut_data_k_gen.get("T",0)
                    except: pass
                
                score_gut_total_k_pdf_gen = g_k_gen * u_k_gen * t_k_gen
                prazo_k_pdf_gen = "N/A"
                if score_gut_total_k_pdf_gen >= 75: prazo_k_pdf_gen = "15 dias"
                elif score_gut_total_k_pdf_gen >= 40: prazo_k_pdf_gen = "30 dias"
                elif score_gut_total_k_pdf_gen >= 20: prazo_k_pdf_gen = "45 dias"
                elif score_gut_total_k_pdf_gen > 0: prazo_k_pdf_gen = "60 dias"
                else: continue
                if prazo_k_pdf_gen != "N/A":
                    gut_cards_pdf_gen_list.append({"Tarefa": pergunta_pdf_k_gen.replace(" [Matriz GUT]", ""),"Prazo": prazo_k_pdf_gen, "Score": score_gut_total_k_pdf_gen})
        if gut_cards_pdf_gen_list:
            gut_cards_pdf_gen_sorted = sorted(gut_cards_pdf_gen_list, key=lambda x_pdf_sort_gen: (int(x_pdf_sort_gen["Prazo"].split(" ")[0]), -x_pdf_sort_gen["Score"])) 
            for card_item_pdf_gen in gut_cards_pdf_gen_sorted: 
                 pdf.multi_cell(0, 6, pdf_safe_text_output(f"Prazo: {card_item_pdf_gen['Prazo']} - Tarefa: {card_item_pdf_gen['Tarefa']} (Score GUT: {card_item_pdf_gen['Score']})"))
        else: pdf.multi_cell(0,6, pdf_safe_text_output("Nenhuma ação prioritária (GUT > 0) identificada."))

        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmpfile_pdf_gerado:
            pdf_path_gerado = tmpfile_pdf_gerado.name
            pdf.output(pdf_path_gerado)
        return pdf_path_gerado
...
if not st.session_state.admin_logado and not st.session_state.cliente_logado:
    aba = st.radio("Você é:", ["Administrador", "Cliente"], horizontal=True, key="tipo_usuario_radio_completo")
...
if aba == "Administrador" and not st.session_state.admin_logado:
    ...
    with st.form("form_admin_login_completo"): 
        usuario_admin_login = st.text_input("Usuário", key="admin_user_login_completo") 
        senha_admin_login = st.text_input("Senha", type="password", key="admin_pass_login_completo")
    ...
    if entrar_admin_login:
        ...
        except Exception as e_login_admin_completo: st.error(f"Erro no login: {e_login_admin_completo}"); st.exception(e_login_admin_completo)
    st.markdown('</div>', unsafe_allow_html=True); st.stop()

if aba == "Cliente" and not st.session_state.cliente_logado:
    st.markdown('<div class="login-container">', unsafe_allow_html=True)
    st.markdown('<h2 class="login-title">Login Cliente</h2>', unsafe_allow_html=True)
    with st.form("form_cliente_login_completo"): 
        cnpj_cli_login = st.text_input("CNPJ", key="cli_cnpj_login_completo") 
        senha_cli_login = st.text_input("Senha", type="password", key="cli_pass_login_completo") 
    ...
    if acessar_cli_login:
        ...
        except FileNotFoundError as e_login_cli_fnf_completo: st.error(f"Arquivo não encontrado: {e_login_cli_fnf_completo.filename}.")
        except pd.errors.EmptyDataError as e_login_cli_empty_completo: st.error(f"Arquivo de usuários ou bloqueados está vazio: {e_login_cli_empty_completo}")
        except Exception as e_login_cli_completo: st.error(f"Erro no login do cliente: {e_login_cli_completo}"); st.exception(e_login_cli_completo)
    st.markdown('</div>', unsafe_allow_html=True); st.stop() 
...
if aba == "Cliente" and st.session_state.cliente_logado:
    try:
        ...
        with st.sidebar.expander("Meu Perfil", expanded=False):
            logo_cliente_path_sidebar = find_client_logo_path(st.session_state.cnpj)
            if logo_cliente_path_sidebar:
                st.image(logo_cliente_path_sidebar, width=100)
            st.write(f"**Empresa:** {st.session_state.user.get('Empresa', 'N/D')}")
            st.write(f"**CNPJ:** {st.session_state.cnpj}")
            st.write(f"**Contato:** {st.session_state.user.get('NomeContato', 'N/D')}")
            st.write(f"**Telefone:** {st.session_state.user.get('Telefone', 'N/D')}")

        DIAGNOSTICO_FORM_ID_KEY_USER_RUNTIME = f"form_id_diagnostico_cliente_{st.session_state.cnpj}"
        ...
        st.session_state.cliente_page = st.sidebar.radio(
            ...
            key="cliente_menu_radio_completo_v2"
        )
        if st.sidebar.button("⬅️ Sair do Portal Cliente"):
            ...
            temp_resp_key_logout = f"temp_respostas_{st.session_state.get(DIAGNOSTICO_FORM_ID_KEY_USER_RUNTIME,'')}"
        ...
        if st.session_state.cliente_page == "Painel Principal":
            ...
            if df_cliente_view_pp.empty: 
                ...
                for idx_cv_pp, row_cv_pp in df_cliente_view_pp.iterrows():
                    with st.expander(f"📅 {row_cv_pp['Data']} - {row_cv_pp['Empresa']}"):
                        ...
                        analise_cli_cv = st.text_area("🧠 Minha Análise sobre este Diagnóstico:", value=analise_cli_val_cv, key=f"analise_cv_completo_{row_cv_pp.name}")
                        if st.button("💾 Salvar Minha Análise", key=f"salvar_analise_cv_completo_{row_cv_pp.name}"):
                            try:
                            ...
                            except Exception as e_save_analise_completo: st.error(f"Erro ao salvar análise: {e_save_analise_completo}")
                ...
                if not df_cliente_view_pp.empty:
                    ...
                    for pergunta_p, resposta_p_val_str in latest_diag_row_painel.items():
                        if isinstance(pergunta_p, str) and "[Matriz GUT]" in pergunta_p:
                            ...
                            except (json.JSONDecodeError, ValueError, TypeError) as e_k_pp: st.warning(f"Erro processar GUT Kanban: '{pergunta_p}'. Erro: {e_k_pp}")
                ...
                if len(df_cliente_view_pp) > 1:
                    ...
                    if len(opcoes_cli) >= 2:
                        ...
                        diag_atual_sel_cli = st.selectbox("Diagnóstico mais recente:", opcoes_cli, index=diag_atual_idx, key="diag_atual_sel_cli_completo")
                        diag_anterior_sel_cli = st.selectbox("Diagnóstico anterior:", opcoes_cli, index=diag_anterior_idx, key="diag_anterior_sel_cli_completo")
            ...
            with st.form(key=f"diagnostico_form_completo_{form_id_sufixo_nd}"):
                if total_perguntas_diag == 0:
                    ...
                    for categoria_diag in categorias_unicas_diag:
                        ...
                        for idx_diag_f, row_diag_f in perguntas_cat_diag.iterrows():
                            ...
                            widget_base_key = f"q_form_completo_{idx_diag_f}" 
                ...
                obs_cli_diag_form = st.text_area("Sua Análise/Observações (opcional):", value=respostas_form_coletadas_nd.get("__obs_cliente__", ""), key=f"obs_cli_diag_completo_{form_id_sufixo_nd}")
                respostas_form_coletadas_nd["__obs_cliente__"] = obs_cli_diag_form
                
                diag_resumo_cli_diag = st.text_area("✍️ Resumo/principais insights (para PDF):", value=respostas_form_coletadas_nd.get("__resumo_cliente__", ""), key=f"diag_resumo_diag_completo_{form_id_sufixo_nd}")
            ...
            if enviar_diagnostico_btn:
                if respondidas_count_diag < total_perguntas_diag: st.warning("Responda todas as perguntas.")
                    ...
                    if pdf_path_final:
                        with open(pdf_path_final, "rb") as f_pdf_final:
                            st.download_button(label="📄 Baixar PDF do Diagnóstico", data=f_pdf_final, 
                                           ...
                                           mime="application/pdf", key="download_pdf_cliente_final_completo")
                    ...
                    st.session_state.cliente_page = "Painel Principal" 
                    st.rerun()
    except Exception as e_cliente_area_completo:
        st.error(f"Ocorreu um erro na área do cliente: {e_cliente_area_completo}")
        st.exception(e_cliente_area_completo) 
...
if aba == "Administrador" and st.session_state.admin_logado:
    ...
    menu_admin = st.sidebar.selectbox( 
        ...
        key="admin_menu_selectbox_completo_final" 
    )
    ...
    try: 
        if menu_admin == "Visão Geral e Diagnósticos":
            ...
            except Exception as e_load_diag_admin_vg:
                st.error(f"ERRO AO CARREGAR DIAGNÓSTICOS (Visão Geral): {e_load_diag_admin_vg}")
                st.exception(e_load_diag_admin_vg)
            ...
            if admin_data_loaded and diagnosticos_df_admin is not None and not diagnosticos_df_admin.empty:
                ...
                empresa_selecionada_vg = st.selectbox(
                    ...
                    key="admin_visao_geral_filtro_empresa_completo"
                )
                ...
                if empresa_selecionada_vg == "Todos os Clientes": # Só faz sentido exportar tudo se estiver vendo todos
                    csv_export_admin_vg = df_filtrado_vg.to_csv(index=False).encode('utf-8') 
                    st.download_button("⬇️ Exportar Todos os Diagnósticos (CSV)", csv_export_admin_vg, file_name=f"diagnosticos_completos.csv", mime="text/csv", key="download_todos_csv_admin_completo")
                st.divider()
                
                st.markdown("#### Detalhar, Comentar e Baixar PDF de Diagnóstico Específico")
                empresas_detalhe_vg = sorted(df_filtrado_vg["Empresa"].astype(str).unique().tolist()) # Usa df_filtrado_vg
                if not empresas_detalhe_vg:
                    st.info("Nenhuma empresa na seleção atual para detalhar.")
                else:
                    # Se uma empresa já estiver filtrada no topo, pré-seleciona aqui
                    default_empresa_detalhe_idx = 0
                    if empresa_selecionada_vg != "Todos os Clientes" and empresa_selecionada_vg in empresas_detalhe_vg:
                        default_empresa_detalhe_idx = empresas_detalhe_vg.index(empresa_selecionada_vg)

                    empresa_selecionada_detalhe_vg = st.selectbox("Selecione uma Empresa para Detalhar:", empresas_detalhe_vg, index=default_empresa_detalhe_idx, key="admin_empresa_filter_detail_completo")
                    ...
                    if not diagnosticos_empresa_detalhe_vg.empty:
                        ...
                        diagnostico_data_selecionada_detalhe_vg = st.selectbox("Selecione a Data do Diagnóstico:", datas_diagnosticos_detalhe_vg, key="admin_data_diagnostico_select_completo")
                        if diagnostico_data_selecionada_detalhe_vg != "Selecione Data...":
                            ...
                            novo_comentario_adm_val_vg = st.text_area("Comentários do Consultor/Admin:", value=comentario_adm_atual_val_vg, key=f"admin_comment_detail_completo_{diagnostico_selecionado_adm_row_vg.name}")
                            if st.button("💾 Salvar Comentário", key=f"save_admin_comment_detail_completo_{diagnostico_selecionado_adm_row_vg.name}"):
                            ...
                            if st.button("📄 Baixar PDF deste Diagnóstico", key=f"download_pdf_admin_completo_{diagnostico_selecionado_adm_row_vg.name}"):
                                try:
                                ...
                                try:
                                    ...
                                    usuario_data_pdf_adm_vg = usuarios_df_pdf_adm_vg[usuarios_df_pdf_adm_vg["CNPJ"] == diagnostico_selecionado_adm_row_vg["CNPJ"]].iloc[0].to_dict()
                                                                 ...
                                                                 "NomeContato": diagnostico_selecionado_adm_row_vg.get("NomeContato",""), #Pega do usuário se possível
                                                                 "Telefone": diagnostico_selecionado_adm_row_vg.get("Telefone","")}
                                ...
                                if pdf_path_admin_dl_vg:
                                    with open(pdf_path_admin_dl_vg, "rb") as f_pdf_adm_dl_vg:
                                        st.download_button(label="Download PDF Confirmado", data=f_pdf_adm_dl_vg, 
                                                       ...
                                                       mime="application/pdf", key=f"confirm_dl_pdf_admin_completo_{diagnostico_selecionado_adm_row_vg.name}")
                ...
                st.warning("Nenhum dado de diagnóstico carregado. A seção 'Visão Geral' não pode ser exibida.")
            ...
            except Exception as e_hist_final: st.error(f"Erro ao carregar histórico: {e_hist_final}")
            ...
            with tabs_perg_admin[0]: 
                ...
                if perguntas_df_admin_edit.empty: st.info("Nenhuma pergunta cadastrada.")
                    ...
                    for i_p_admin, row_p_admin in perguntas_df_admin_edit.iterrows():
                        ...
                        with cols_p_admin[0]:
                            nova_p_text_admin = st.text_input("Pergunta", value=str(row_p_admin["Pergunta"]), key=f"edit_p_txt_completo_{i_p_admin}")
                        with cols_p_admin[1]:
                            nova_cat_text_admin = st.text_input("Categoria", value=str(row_p_admin.get("Categoria", "Geral")), key=f"edit_p_cat_completo_{i_p_admin}")
                        with cols_p_admin[2]:
                            st.write("") 
                            if st.button("💾", key=f"salvar_p_adm_completo_{i_p_admin}", help="Salvar"):
                        ...
                        with cols_p_admin[3]:
                            st.write("") 
                            if st.button("🗑️", key=f"deletar_p_adm_completo_{i_p_admin}", help="Deletar"):
            ...
            with tabs_perg_admin[1]: 
                with st.form("form_nova_pergunta_admin_completo"):
                    st.subheader("➕ Adicionar Nova Pergunta")
                    nova_p_form_txt_admin = st.text_input("Texto da Pergunta", key="nova_p_input_admin_txt_completo")
                    try:
                    ...
                    cat_selecionada = st.selectbox("Categoria:", cat_options, key="cat_select_admin_new_q_completo")
                    
                    if cat_selecionada == "Nova Categoria":
                        nova_cat_form_admin = st.text_input("Nome da Nova Categoria:", key="nova_cat_input_admin_new_q_completo")
                    ...
                    tipo_p_form_admin = st.selectbox("Tipo de Pergunta", 
                                                 ...
                                                 key="tipo_p_select_admin_new_q_completo")
                ...
                for idx_gc, row_gc in usuarios_clientes_df.iterrows():
                    with st.expander(f"{row_gc.get('Empresa','N/A')} (CNPJ: {row_gc['CNPJ']})"):
                        ...
                        with cols_edit_cli[0]:
                            st.text_input("CNPJ (não editável)", value=row_gc['CNPJ'], disabled=True, key=f"cnpj_gc_completo_{idx_gc}")
                            nova_senha_gc = st.text_input("Nova Senha (deixe em branco para não alterar)", type="password", key=f"senha_gc_completo_{idx_gc}")
                            nome_empresa_gc = st.text_input("Nome Empresa", value=row_gc.get('Empresa',""), key=f"empresa_gc_completo_{idx_gc}")
                        with cols_edit_cli[1]:
                            nome_contato_gc = st.text_input("Nome Contato", value=row_gc.get("NomeContato", ""), key=f"nomec_gc_completo_{idx_gc}")
                            telefone_gc = st.text_input("Telefone", value=row_gc.get("Telefone", ""), key=f"tel_gc_completo_{idx_gc}")
                            ...
                            uploaded_logo_gc = st.file_uploader("Alterar/Adicionar Logo", type=["png", "jpg", "jpeg"], key=f"logo_gc_completo_{idx_gc}")

                        if st.button("💾 Salvar Alterações do Cliente", key=f"save_gc_completo_{idx_gc}"):
                            ...
                            if uploaded_logo_gc is not None:
                                ...
                                clean_cnpj_gc = sanitize_column_name(str(row_gc['CNPJ'])) # Limpa CNPJ para nome de arquivo
                                for ext_old in ["png", "jpg", "jpeg"]: 
                                    old_path = os.path.join(LOGOS_DIR, f"{clean_cnpj_gc}_logo.{ext_old}")
                                    if os.path.exists(old_path): os.remove(old_path)
                                file_extension = uploaded_logo_gc.name.split('.')[-1].lower()
                                logo_save_path_gc = os.path.join(LOGOS_DIR, f"{clean_cnpj_gc}_logo.{file_extension}")
            ...
            with st.form("form_novo_cliente_admin_completo"):
                ...
                if novo_cnpj_gc_form and nova_senha_gc_form and nova_empresa_gc_form:
                    if novo_cnpj_gc_form in usuarios_clientes_df["CNPJ"].astype(str).values:
                        ...
                        if nova_logo_gc_form is not None:
                            if not os.path.exists(LOGOS_DIR): os.makedirs(LOGOS_DIR)
                            clean_cnpj_new_gc = sanitize_column_name(str(novo_cnpj_gc_form))
                            file_extension_new = nova_logo_gc_form.name.split('.')[-1].lower()
                            logo_save_path_new_gc = os.path.join(LOGOS_DIR, f"{clean_cnpj_new_gc}_logo.{file_extension_new}")
            ...
            with col_block:
                cnpj_para_bloquear = st.selectbox("Bloquear CNPJ:", [""] + usuarios_clientes_df["CNPJ"].astype(str).unique().tolist(), key="block_cnpj_completo")
                if st.button("Bloquear Selecionado", key="btn_block_completo") and cnpj_para_bloquear:
            ...
            with col_unblock:
                cnpj_para_desbloquear = st.selectbox("Desbloquear CNPJ:", [""] + bloqueados_df_adm["CNPJ"].astype(str).unique().tolist(), key="unblock_cnpj_completo")
                if st.button("Desbloquear Selecionado", key="btn_unblock_completo") and cnpj_para_desbloquear:
            ...
            with st.form("form_novo_admin_manage_completo"):
            ...
            if not admins_df_manage.empty:
                admin_para_remover_manage = st.selectbox("Remover Admin:", options=[""] + admins_df_manage["Usuario"].tolist(), key="remove_admin_select_manage_completo")
                if st.button("Remover Admin Selecionado", type="primary", key="btn_remove_admin_completo") and admin_para_remover_manage:
    ...
    except Exception as e_admin_area_completo_final:
        st.error(f"Ocorreu um erro crítico na área administrativa: {e_admin_area_completo_final}")
        st.exception(e_admin_area_completo_final)
