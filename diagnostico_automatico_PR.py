...
.stTextInput>div>input:focus, .stTextArea>div>textarea:focus, .stDateInput>div>input:focus {
...
if aba == "Cliente" and st.session_state.cliente_logado:
    ...
    try: current_idx_cli = menu_options_cli.index(st.session_state.cliente_page) # Tentar manter a página se o nome for igual
    except ValueError: # Se o nome da página mudou ou é inválido, recalcula o índice
        page_map = {"Instruções": "📖 Instruções", "Novo Diagnóstico": "📋 Novo Diagnóstico", "Painel Principal": "🏠 Painel Principal"}
        try: current_idx_cli = menu_options_cli.index(page_map.get(st.session_state.cliente_page, menu_options_cli[0]))
        except ValueError: current_idx_cli = 0
        st.session_state.cliente_page = menu_options_cli[current_idx_cli] # Garante que cliente_page seja um valor válido do menu
    
    selected_page_cli_raw = st.sidebar.radio("Menu Cliente", menu_options_cli, index=current_idx_cli, key="cli_menu_v14") 
    # Extrair o nome da página sem o emoji para st.session_state.cliente_page
    selected_page_cli_clean = selected_page_cli_raw.split(" ", 1)[1] if " " in selected_page_cli_raw else selected_page_cli_raw
    ...
    if st.session_state.cliente_page == "Instruções":
            ...
            if df_cliente_diags.empty: st.info("Nenhum diagnóstico anterior.")
                ...
                for idx_row_diag, row_diag_data in df_cliente_diags.iterrows():
                    with st.expander(f"📅 {row_diag_data['Data']} - {row_diag_data['Empresa']}"):
                        st.markdown('<div class="custom-card">', unsafe_allow_html=True) # CARD INÍCIO
                        ...
                        if st.button("📄 Baixar PDF deste Diagnóstico", key=f"dl_pdf_antigo_v14_{idx_row_diag}", icon="📄"): 
                            ...
                            if pdf_path_antigo:
                                with open(pdf_path_antigo, "rb") as f_antigo:
                                    # O download button precisa ser único e só aparece após o clique.
                                    # Para evitar problemas de estado, vamos mudar a chave do botão de download real
        ...
        if st.button("✔️ Concluir e Enviar Diagnóstico", key="enviar_diag_final_cliente_v14", icon="✔️", use_container_width=True): 
...
if aba == "Administrador" and st.session_state.admin_logado:
    try:
        st.sidebar.image("https://via.placeholder.com/200x80.png?text=Logo+Admin", use_column_width=True)
    ...
    formatted_admin_options = [f"{emoji} {text}" for text, emoji in menu_admin_options.items()]
    ...
    menu_admin = selected_admin_menu_raw.split(" ", 1)[1]
    ...
    if menu_admin == "Visão Geral e Diagnósticos":
                ...
                for i_p_admin, row_p_admin in perguntas_df_admin_gp.iterrows():
                    with st.container(): # Envolve cada pergunta em um container para aplicar estilo de card
                        ...
                        with st.expander("✏️ Editar Pergunta"):
                            ...
                            if col_btn1.button("💾", key=f"salvar_p_adm_v14_gp_{i_p_admin}", help="Salvar Alterações", icon="💾"):
                            ...
                            if col_btn2.button("🗑️", key=f"deletar_p_adm_v14_gp_{i_p_admin}", help="Deletar Pergunta", icon="🗑️"):
        ...
        if df_analises_existentes_admin.empty: st.info("Nenhuma análise cadastrada.")
            ...
            if st.button("🗑️ Deletar Análise Selecionada", key="btn_del_analise_v14_ga", icon="🗑️", type="secondary"): 
