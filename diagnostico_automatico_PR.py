...
# from fpdf.enums import XPos, YPos # REMOVIDO para simplificar
import tempfile
import re
import json
...
def gerar_pdf_diagnostico_completo(diag_data, user_data, perguntas_df, respostas_coletadas, medias_cat, analises_df):
    try:
        ...
        pdf.multi_cell(w=0, h=10, text=pdf_safe_text_output(f"Diagnóstico Empresarial - {empresa_nome}"), border=0, align='C')
        pdf.ln() # Adicionado para garantir quebra de linha
        pdf.ln(5)
        pdf.set_font("Arial", size=10)
        pdf.multi_cell(w=0, h=7, text=pdf_safe_text_output(f"Data: {diag_data.get('Data','N/D')} | Empresa: {empresa_nome} (CNPJ: {cnpj_pdf})"), border=0, align='L')
        pdf.ln()
        if user_data.get("NomeContato"): 
            pdf.multi_cell(w=0, h=7, text=pdf_safe_text_output(f"Contato: {user_data.get('NomeContato')}"), border=0, align='L')
            pdf.ln()
        if user_data.get("Telefone"): 
            pdf.multi_cell(w=0, h=7, text=pdf_safe_text_output(f"Telefone: {user_data.get('Telefone')}"), border=0, align='L')
            pdf.ln()
        pdf.ln(3)
        pdf.multi_cell(w=0, h=7, text=pdf_safe_text_output(f"Média Geral: {diag_data.get('Média Geral','N/A')} | GUT Média: {diag_data.get('GUT Média','N/A')}"), border=0, align='L')
        pdf.ln()
        pdf.ln(3)

        if medias_cat:
            pdf.set_font("Arial", 'B', 11)
            pdf.multi_cell(w=0, h=7, text=pdf_safe_text_output("Médias por Categoria:"), border=0, align='L')
            pdf.ln()
            pdf.set_font("Arial", size=10)
            for cat, media in sorted(medias_cat.items()): 
                pdf.multi_cell(w=0, h=6, text=pdf_safe_text_output(f"  - {cat}: {media:.2f}"), border=0, align='L')
                pdf.ln()
            pdf.ln(5)
        ...
        for titulo, campo in [("Resumo (Cliente):", "Diagnóstico"), ("Análise (Cliente):", "Análise do Cliente"), ("Comentários (Consultor):", "Comentarios_Admin")]:
            ...
            if valor and not pd.isna(valor) and str(valor).strip():
                ...
                pdf.multi_cell(w=0, h=7, text=pdf_safe_text_output(titulo), border=0, align='L')
                pdf.ln()
                pdf.set_font("Arial", size=10)
                pdf.multi_cell(w=0, h=7, text=pdf_safe_text_output(str(valor)), border=0, align='L')
                pdf.ln()
                pdf.ln(3)

        pdf.set_font("Arial", 'B', 12)
        pdf.multi_cell(w=0, h=10, text=pdf_safe_text_output("Respostas Detalhadas e Análises:"), border=0, align='L')
        pdf.ln()
        ...
        for categoria in categorias:
            ...
            pdf.multi_cell(w=0, h=7, text=pdf_safe_text_output(f"Categoria: {categoria}"), border=0, align='L')
            pdf.ln()
            ...
            for _, p_row in perg_cat.iterrows():
                ...
                if "[Matriz GUT]" in p_texto:
                    ...
                    score = g*u*t
                    pdf.multi_cell(w=0, h=6, text=pdf_safe_text_output(f"  - {p_texto.replace(' [Matriz GUT]','')}: G={g}, U={u}, T={t} (Score: {score})"), border=0, align='L')
                    pdf.ln()
                    analise_texto = obter_analise_para_resposta(p_texto, score, analises_df)
                else:
                    pdf.multi_cell(w=0, h=6, text=pdf_safe_text_output(f"  - {p_texto}: {resp}"), border=0, align='L')
                    pdf.ln()
                    analise_texto = obter_analise_para_resposta(p_texto, resp, analises_df)
                if analise_texto:
                    pdf.set_font("Arial", 'I', 8); pdf.set_text_color(100,100,100)
                    pdf.multi_cell(w=0, h=5, text=pdf_safe_text_output(f"    Análise: {analise_texto}"), border=0, align='L')
                    pdf.ln()
                    pdf.set_text_color(0,0,0); pdf.set_font("Arial", size=9)
            pdf.ln(2)
        pdf.ln(3)
        pdf.add_page(); pdf.set_font("Arial", 'B', 12)
        # Para pdf.cell, o 5º argumento é ln (0, 1, 2)
        ...
        if gut_cards:
            ...
            for card in sorted_cards: 
                pdf.multi_cell(w=0, h=6, text=pdf_safe_text_output(f"Prazo: {card['Prazo']} - Tarefa: {card['Tarefa']} (Score GUT: {card['Score']})"), border=0, align='L')
                pdf.ln()
        else: 
            pdf.multi_cell(w=0, h=6, text=pdf_safe_text_output("Nenhuma ação prioritária (GUT > 0) identificada."), border=0, align='L')
            pdf.ln()
...
def gerar_pdf_historico(df_historico_filtrado, titulo="Histórico de Ações"):
    ...
    pdf.add_page()
    pdf.set_font("Arial", "B", 16)
    pdf.cell(w=0, h=10, txt=pdf_safe_text_output(titulo), border=0, ln=1, align="C") # ln=1 para nova linha
    pdf.ln(10)

    pdf.set_font("Arial", "B", 10)
    col_widths_config = {"Data": 35, "CNPJ": 35, "Ação": 40, "Descrição": 75} 
    ...
    # Ajustar a largura da coluna 'Descrição' para preencher o espaço restante
    current_total_width = sum(col_widths_config.get(h,0) for h in headers_to_print_hist if h != "Descrição")
    desc_width = page_width_effective - current_total_width
    if desc_width < 20: # Largura mínima para descrição
        desc_width = 20 
        # Se a descrição for muito pequena, pode ser necessário reajustar outras colunas ou aceitar overflow
    col_widths_config["Descrição"] = desc_width


    for header in headers_to_print_hist:
        pdf.cell(w=col_widths_config.get(header, 30), h=10, txt=pdf_safe_text_output(header), border=1, align="C")
    pdf.ln() 
    ...
    for _, row in df_historico_filtrado.iterrows():
        ...
        for header_calc in headers_to_print_hist:
            cell_text_for_calc = str(row.get(header_calc, ""))
            col_w_calc = col_widths_config.get(header_calc, 30)
            
            if cell_text_for_calc and col_w_calc > 0:
                # Salvar posição e fonte para cálculo de altura de multi_cell
                save_x, save_y = pdf.get_x(), pdf.get_y()
                save_font_family, save_font_style, save_font_size = pdf.font_family, pdf.font_style, pdf.font_size_pt
                
                pdf.set_font("Arial", "", 8) # Garantir a fonte correta para cálculo
                lines = pdf.multi_cell(
                    w=col_w_calc, 
                    h=line_height_text, 
                    text=pdf_safe_text_output(cell_text_for_calc), 
                    border=0, 
                    align="L", 
                    split_only=True
                )
                pdf.set_font(save_font_family, save_font_style, save_font_size) # Restaurar fonte
                pdf.set_xy(save_x, save_y) # Restaurar posição
                
                height_needed = len(lines) * line_height_text
                max_h_for_this_row = max(max_h_for_this_row, height_needed)
        
        max_h_for_this_row = max(max_h_for_this_row, line_height_text) # Garantir altura mínima

        # Desenhar as células da linha
        current_x_for_cell = pdf.l_margin
        for header_draw in headers_to_print_hist:
            cell_text_to_draw = str(row.get(header_draw, ""))
            pdf.set_xy(current_x_for_cell, row_start_y) 
            
            col_w_draw = col_widths_config.get(header_draw, 30)
            ...
            pdf.multi_cell(
                ...
                border=1,
                align="L"
                # Não usar new_x/new_y aqui, pois estamos controlando manualmente
            )
            current_x_for_cell += col_w_draw
            
        pdf.set_y(row_start_y + max_h_for_this_row)
...
# --- Lógica de Login e Navegação Principal --- (Mantida como estava, apenas atualizando chaves para evitar conflitos de widget)
if st.session_state.get("trigger_rerun_global"): st.session_state.trigger_rerun_global = False; st.rerun()

if not st.session_state.admin_logado and not st.session_state.cliente_logado:
    aba = st.radio("Você é:", ["Administrador", "Cliente"], horizontal=True, key="tipo_usuario_radio_v17") 
...
if aba == "Administrador" and not st.session_state.admin_logado:
    ...
    with st.form("form_admin_login_v17"): 
        u = st.text_input("Usuário", key="admin_u_v17"); p = st.text_input("Senha", type="password", key="admin_p_v17") 
...
if aba == "Cliente" and not st.session_state.cliente_logado:
    ...
    with st.form("form_cliente_login_v17"): 
        c = st.text_input("CNPJ", key="cli_c_v17", value=st.session_state.get("last_cnpj_input","")) 
        s = st.text_input("Senha", type="password", key="cli_s_v17") 
        if st.form_submit_button("Entrar"):
...
# --- ÁREA DO CLIENTE LOGADO --- (Chaves de widget atualizadas para v17)
if aba == "Cliente" and st.session_state.cliente_logado:
    if "user" not in st.session_state or st.session_state.user is None or \
       "cnpj" not in st.session_state or st.session_state.cnpj is None:
        ...
        # Limpeza e reinicialização do estado da sessão do cliente
    ...
    selected_page_cli_raw_val = st.sidebar.radio("Menu Cliente", menu_options_cli_val, index=current_idx_cli_val, key="cli_menu_v17") 
    ...
    if st.sidebar.button("⬅️ Sair do Portal Cliente", key="logout_cliente_v17"): 
    ...
    if st.session_state.cliente_page == "Instruções":
        ...
        if st.session_state.user:
            ...
            if pode_fazer_novo_inst_page_val:
                st.session_state.confirmou_instrucoes_checkbox_cliente = st.checkbox("Declaro que li e compreendi todas as instruções fornecidas para a realização deste diagnóstico.", value=st.session_state.get("confirmou_instrucoes_checkbox_cliente", False), key="confirma_leitura_inst_v17_cb") 
                if st.button("Prosseguir para o Diagnóstico", key="btn_instrucoes_v17_prosseguir", disabled=not st.session_state.confirmou_instrucoes_checkbox_cliente): 
                ...
                if st.button("Ir para o Painel Principal", key="ir_painel_inst_sem_diag_v17"): st.session_state.cliente_page = "Painel Principal"; st.rerun() 
            ...
            if st.session_state.pdf_gerado_path and st.session_state.pdf_gerado_filename:
                try:
                    with open(st.session_state.pdf_gerado_path, "rb") as f_pdf:
                        st.download_button(label="📄 Baixar PDF do Diagnóstico Recém-Enviado", data=f_pdf, file_name=st.session_state.pdf_gerado_filename, mime="application/pdf", key="dl_novo_diag_painel_v17_pp") 
        ...
        try:
            ...
            elif not os.path.exists(arquivo_csv) or os.path.getsize(arquivo_csv) == 0:
                st.info(f"Nenhum diagnóstico anterior para exibir.")
            else:
        ...
        if df_cliente_diags.empty:
            ...
            for idx_row_diag, row_diag_data in df_cliente_diags.iterrows():
                with st.expander(f"📅 {row_diag_data.get('Data','Data Indisponível')} - {row_diag_data.get('Empresa','Empresa Indisponível')}"):
                    # ... (conteúdo do expander do cliente mantido, apenas chaves de widget atualizadas para v17)
                    ...
                    analise_cli_val_cv_painel = row_diag_data.get("Análise do Cliente", ""); analise_cli_cv_input = st.text_area("🧠 Minha Análise sobre este Diagnóstico:", value=analise_cli_val_cv_painel, key=f"analise_cv_painel_v17_pp_{idx_row_diag}") 
                    if st.button("💾 Salvar Minha Análise", key=f"salvar_analise_cv_painel_v17_pp_{idx_row_diag}"): 
                        try:
                    ...
                    if st.button("📄 Baixar PDF deste Diagnóstico", key=f"dl_pdf_antigo_v17_pp_{idx_row_diag}"): 
                        ...
                        if pdf_path_antigo:
                            try:
                                with open(pdf_path_antigo, "rb") as f_antigo: st.download_button("Download PDF Confirmado", f_antigo, file_name=f"diag_{sanitize_column_name(row_diag_data['Empresa'])}_{str(row_diag_data['Data']).replace(':','-').replace(' ','_')}.pdf", mime="application/pdf", key=f"dl_confirm_antigo_v17_pp_{idx_row_diag}") 
                    ...
                    st.divider() 
            # ... (Restante da área do cliente mantido, com chaves de widget atualizadas para v17 onde aplicável)
            ...
            if len(df_cliente_diags) >= 1:
                ...
                diagnosticos_selecionados_comp = st.multiselect("Selecione os diagnósticos para comparar:", options=datas_opts_comp_list, default=default_selection_comp, key="cliente_multiselect_comparacao_v17_pp") 
        ...
        # ... (conteúdo do Novo Diagnóstico mantido, apenas chaves de widget atualizadas para v17)
        ...
        if not pode_fazer_novo_form:
            st.warning("Você não tem diagnósticos disponíveis.")
            if st.button("Voltar ao Painel Principal", key="voltar_painel_novo_diag_bloq_v17_nd"): st.session_state.cliente_page = "Painel Principal"; st.rerun()
            st.stop()
        elif not confirmou_inst_form: 
            st.warning("Por favor, confirme a leitura das instruções.")
            if st.button("Ir para Instruções", key="ir_instrucoes_novo_diag_v17_nd"): st.session_state.cliente_page = "Instruções"; st.rerun()
            st.stop()
        if st.session_state.diagnostico_enviado_sucesso:
            ...
            if st.session_state.pdf_gerado_path and st.session_state.pdf_gerado_filename:
                try:
                    with open(st.session_state.pdf_gerado_path, "rb") as f_pdf_dl_sucesso:
                        st.download_button(label="📄 Baixar PDF do Diagnóstico Enviado", data=f_pdf_dl_sucesso, file_name=st.session_state.pdf_gerado_filename, mime="application/pdf", key="dl_pdf_sucesso_novo_diag_v17_nd")
            ...
            if st.button("Ir para o Painel Principal", key="ir_painel_apos_envio_sucesso_v17_nd"):
        ...
        try:
            if not os.path.exists(perguntas_csv) or os.path.getsize(perguntas_csv) == 0: st.error(f"Arquivo de perguntas ('{perguntas_csv}') não encontrado ou vazio."); st.stop()
        ...
        for categoria_novo in sorted(perguntas_df_formulario["Categoria"].unique()):
            ...
            for idx_novo, row_q_novo in perg_cat_df_novo.iterrows():
                ...
                with cols_q_feedback[0]:
                    # ... (lógica de widgets de perguntas mantida)
        ...
        if st.button("✔️ Concluir e Enviar Diagnóstico", key="enviar_diag_final_cliente_v17_nd"): 
        ...
        # ... (conteúdo de Notificações mantido, apenas chaves de widget atualizadas para v17)
        ...
        if df_notif_cliente_view.empty: st.info("Você não tem nenhuma notificação no momento.")
            ...
            if ids_nao_lidas_para_marcar_view:
                if 'notif_page_loaded_once_v17_c' not in st.session_state:  
                    if marcar_notificacoes_como_lidas(st.session_state.cnpj, ids_notificacoes=ids_nao_lidas_para_marcar_view):
                        st.session_state.notif_page_loaded_once_v17_c = True; 
                        st.rerun() 
            elif 'notif_page_loaded_once_v17_c' in st.session_state and not ids_nao_lidas_para_marcar_view: 
                del st.session_state.notif_page_loaded_once_v17_c 


# --- ÁREA DO ADMINISTRADOR LOGADO --- (Chaves de widget atualizadas para v17 e nova funcionalidade)
if aba == "Administrador" and st.session_state.admin_logado:
    try:
        ...
        if st.sidebar.button("🚪 Sair do Painel Admin", key="logout_admin_v17_adm"): st.session_state.admin_logado = False; st.rerun() 
        ...
        menu_admin = st.sidebar.selectbox("Funcionalidades Admin:", menu_admin_options, key="admin_menu_selectbox_v17_adm") 
        st.header(f"{menu_admin.split(' ')[0]} {menu_admin.split(' ', 1)[1]}")
        df_usuarios_admin_geral = pd.DataFrame(columns=colunas_base_usuarios) # Definir antes do try
        try:
        ...
        diagnosticos_df_admin_orig_view = pd.DataFrame() # Definir antes do try
        ...
        try:
            if menu_admin == "📊 Visão Geral e Diagnósticos":
                # ... (conteúdo de Visão Geral mantido, apenas chaves de widget atualizadas para v17)
                ...
                with col_f1_vg: emp_sel_admin_vg = st.selectbox("Filtrar por Empresa:", ["Todos os Clientes"] + empresas_lista_admin_filtro_vg, key="admin_filtro_emp_v17_vg") 
                with col_f2_vg: dt_ini_admin_vg = st.date_input("Data Início dos Diagnósticos:", value=None, key="admin_dt_ini_v17_vg") 
                with col_f3_vg: dt_fim_admin_vg = st.date_input("Data Fim dos Diagnósticos:", value=None, key="admin_dt_fim_v17_vg") 
                st.divider()
                ...
                st.divider()
                if not admin_data_carregada_view_sucesso or df_diagnosticos_admin_orig_view.empty : st.warning("Nenhum dado de diagnóstico foi carregado.")
                    ...
                    if not df_diagnosticos_filtrados_view_final_vg.empty:
                        ...
                        diag_selecionado_str_admin = st.selectbox("Selecione um Diagnóstico para Detalhar:", [""] + diagnosticos_para_detalhe_admin, key="admin_select_diag_detalhe_v17") 
                        if diag_selecionado_str_admin:
                            try:
                                ...
                                novos_comentarios_admin_det = st.text_area("Comentários do Consultor:", value=comentarios_admin_atuais_det if pd.notna(comentarios_admin_atuais_det) else "", key=f"com_admin_det_v17_{diag_original_index_admin}")
                                if st.button("Salvar Comentários do Consultor 💬", key=f"save_com_admin_det_v17_{diag_original_index_admin}"):
                                ...
                                if st.button("📄 Baixar PDF deste Diagnóstico", key=f"dl_pdf_admin_detalhe_v17_{diag_original_index_admin}"): 
                ...
                # ... (conteúdo de Status dos Clientes mantido, apenas chaves de widget atualizadas para v17)
                ...
                emp_sel_status_view = st.selectbox("Filtrar por Empresa:", empresas_status_list_view, key="status_emp_sel_v17") 
                ...
                emp_sel_hu = col_hu_f1.selectbox("Filtrar por Empresa:", empresas_hist_list_hu, key="hist_emp_sel_v17_hu_adm")
                termo_busca_hu = col_hu_f2.text_input("Buscar por Nome do Contato, CNPJ, Ação ou Descrição:", key="hist_termo_busca_v17_hu_adm")
                ...
                if termo_busca_hu.strip() and not df_historico_filtrado_view_hu.empty : # Aplicar busca apenas se houver histórico filtrado por empresa ou geral
                ...
                if not df_historico_filtrado_view_hu.empty:
                    ...
                    if st.button("📄 Baixar Histórico Filtrado (PDF)", key="download_hist_filtrado_pdf_v17_hu_adm"):
                        ...
                        if pdf_path_hist:
                            with open(pdf_path_hist, "rb") as f_pdf_hist: 
                                st.download_button(label="Download Confirmado", data=f_pdf_hist, file_name=titulo_pdf_hist, mime="application/pdf", key="confirm_download_hist_pdf_v17_hu_adm")
                            try: os.remove(pdf_path_hist) 
                            except: pass
                    
                    # --- NOVA FUNCIONALIDADE: Excluir Histórico da Empresa Filtrada ---
                    if emp_sel_hu != "Todas" and not df_historico_filtrado_view_hu.empty:
                        st.markdown("---")
                        ...
                        with st.expander(f"⚠️ ATENÇÃO: Excluir Histórico Completo da Empresa '{emp_sel_hu}'"):
                            ...
                            if st.checkbox(f"Confirmo que desejo excluir TODO o histórico da empresa '{emp_sel_hu}'.", key=f"confirm_delete_hist_emp_{emp_sel_hu}_v17"):
                                if st.button(f"🗑️ Excluir Histórico de '{emp_sel_hu}' AGORA", type="primary", key=f"btn_delete_hist_emp_{emp_sel_hu}_v17"):
                                    try:
                ...
                # ... (conteúdo de Gerenciar Perguntas mantido, apenas chaves de widget atualizadas para v17)
                ...
                with tabs_perg_admin[0]:
                    if perguntas_df_admin_gp.empty: st.info("Nenhuma pergunta cadastrada.")
                        ...
                        for i_p_admin, row_p_admin in perguntas_df_admin_gp.iterrows():
                            ...
                            with cols_p_admin[0]: nova_p_text_admin = st.text_input("Pergunta", value=str(row_p_admin["Pergunta"]), key=f"edit_p_txt_v17_gp_{i_p_admin}") 
                            with cols_p_admin[1]: nova_cat_text_admin = st.text_input("Categoria", value=str(row_p_admin.get("Categoria", "Geral")), key=f"edit_p_cat_v17_gp_{i_p_admin}") 
                            with cols_p_admin[2]:
                                st.markdown("<br/>", unsafe_allow_html=True) 
                                if st.button("💾", key=f"salvar_p_adm_v17_gp_{i_p_admin}", help="Salvar Alterações"): 
                            ...
                            with cols_p_admin[3]:
                                st.markdown("<br/>", unsafe_allow_html=True) 
                                if st.button("🗑️", key=f"deletar_p_adm_v17_gp_{i_p_admin}", help="Deletar Pergunta"): 
                ...
                with tabs_perg_admin[1]:
                    with st.form("form_nova_pergunta_admin_v17_gp"): 
                        st.subheader("➕ Adicionar Nova Pergunta"); nova_p_form_txt_admin = st.text_input("Texto da Pergunta (sem incluir o tipo, ex: 'Qual o nível de satisfação?')", key="nova_p_input_admin_txt_v17_gp") 
                        cat_existentes_gp = sorted(list(perguntas_df_admin_gp['Categoria'].astype(str).unique())) if not perguntas_df_admin_gp.empty and "Categoria" in perguntas_df_admin_gp.columns else []; cat_options_gp = ["Nova Categoria"] + cat_existentes_gp; cat_selecionada_gp = st.selectbox("Categoria:", cat_options_gp, key="cat_select_admin_new_q_v17_gp") 
                        nova_cat_form_admin_gp = st.text_input("Nome da Nova Categoria (se 'Nova Categoria' selecionada acima):", key="nova_cat_input_admin_new_q_v17_gp") if cat_selecionada_gp == "Nova Categoria" else cat_selecionada_gp 
                        tipo_p_form_admin = st.selectbox("Tipo de Pergunta (será adicionado ao final do texto da pergunta):", ["Pontuação (0-10)", "Pontuação (0-5)", "Texto Aberto", "Escala", "[Matriz GUT]"], key="tipo_p_select_admin_new_q_v17_gp") 
                ...
                # ... (conteúdo de Gerenciar Análises mantido, apenas chaves de widget atualizadas para v17)
                ...
                if df_perguntas_formulario_admin.empty: st.warning("Nenhuma pergunta cadastrada no formulário.")
                else:
                    lista_perguntas_txt_admin = [""] + df_perguntas_formulario_admin["Pergunta"].unique().tolist(); pergunta_selecionada_analise_admin = st.selectbox("Selecione a Pergunta para adicionar análise:", lista_perguntas_txt_admin, key="sel_perg_analise_v17_ga") 
                    if pergunta_selecionada_analise_admin:
                        st.caption(f"Pergunta selecionada: {pergunta_selecionada_analise_admin}"); tipo_condicao_analise_display_admin = st.selectbox("Tipo de Condição para a Análise:", ["Faixa Numérica (p/ Pontuação 0-X)", "Valor Exato (p/ Escala)", "Faixa de Score (p/ Matriz GUT)", "Análise Padrão (default para a pergunta)"], key="tipo_cond_analise_v17_ga") 
                        ...
                        if tipo_condicao_csv_val_admin == "FaixaNumerica": cols_faixa_ui_admin = st.columns(2); cond_val_min_ui_admin = cols_faixa_ui_admin[0].number_input("Valor Mínimo da Faixa", step=1.0, format="%.2f", key="cond_min_analise_v17_ga"); cond_val_max_ui_admin = cols_faixa_ui_admin[1].number_input("Valor Máximo da Faixa", step=1.0, format="%.2f", key="cond_max_analise_v17_ga") 
                        elif tipo_condicao_csv_val_admin == "ValorExatoEscala": cond_val_exato_ui_admin = st.text_input("Valor Exato da Escala (ex: Baixo, Médio, Alto)", key="cond_exato_analise_v17_ga") 
                        elif tipo_condicao_csv_val_admin == "ScoreGUT": cols_faixa_gut_ui_admin = st.columns(2); cond_val_min_ui_admin = cols_faixa_gut_ui_admin[0].number_input("Score GUT Mínimo", step=1, key="cond_min_gut_analise_v17_ga"); cond_val_max_ui_admin = cols_faixa_gut_ui_admin[1].number_input("Score GUT Máximo (opcional)", value=0.0, step=1.0, format="%.0f", key="cond_max_gut_analise_v17_ga")  
                        texto_analise_nova_ui_admin = st.text_area("Texto da Análise:", height=150, key="txt_analise_nova_v17_ga") 
                        if st.button("💾 Salvar Nova Análise", key="salvar_analise_pergunta_v17_ga"): 
                            if texto_analise_nova_ui_admin.strip(): nova_id_analise_admin = str(uuid.uuid4()); nova_entrada_analise_admin = { "ID_Analise": nova_id_analise_admin, "TextoPerguntaOriginal": pergunta_selecionada_analise_admin, "TipoCondicao": tipo_condicao_csv_val_admin, "CondicaoValorMin": cond_val_min_ui_admin if cond_val_min_ui_admin is not None else pd.NA, "CondicaoValorMax": cond_val_max_ui_admin if cond_val_max_ui_admin is not None and cond_val_max_ui_admin !=0 else pd.NA, "CondicaoValorExato": cond_val_exato_ui_admin if cond_val_exato_ui_admin else pd.NA, "TextoAnalise": texto_analise_nova_ui_admin }; df_analises_existentes_admin = pd.concat([df_analises_existentes_admin, pd.DataFrame([nova_entrada_analise_admin])], ignore_index=True); df_analises_existentes_admin.to_csv(analises_perguntas_csv, index=False, encoding='utf-8'); st.success(f"Nova análise salva!"); st.rerun()
                ...
                if df_analises_para_exibir.empty: st.info("Nenhuma análise cadastrada.")
                    ...
                    analise_del_id_admin_view = st.selectbox("Deletar Análise por ID:", [""] + df_analises_para_exibir["ID_Analise"].astype(str).tolist(), key="del_analise_id_v17_ga_view") 
                    if st.button("🗑️ Deletar Análise Selecionada", key="btn_del_analise_v17_ga_view", type="primary") and analise_del_id_admin_view: df_analises_para_exibir = df_analises_para_exibir[df_analises_para_exibir["ID_Analise"] != analise_del_id_admin_view]; df_analises_para_exibir.to_csv(analises_perguntas_csv, index=False, encoding='utf-8'); st.warning("Análise deletada."); st.rerun() 

            elif menu_admin == "✍️ Gerenciar Instruções Clientes":
                # ... (conteúdo de Gerenciar Instruções mantido, apenas chaves de widget atualizadas para v17)
                ...
                try:
                    if os.path.exists(instrucoes_txt_file) and os.path.getsize(instrucoes_txt_file) > 0:
                        ...
                        with open(instrucoes_txt_file, "w", encoding="utf-8") as f: 
                            default_text = """**Bem-vindo ao Portal de Diagnóstico Empresarial!** (Conteúdo padrão)""" 
                            f.write(default_text) 
                            current_instructions = default_text
                except Exception as e: st.error(f"Erro ao ler arquivo de instruções: {e}")
                edited_instructions = st.text_area("Edite as instruções para os clientes (use Markdown):", value=current_instructions, height=400, key="admin_edit_instructions_ta_v17") 
                if st.button("💾 Salvar Instruções", key="admin_save_instructions_btn_v17"): 
                    try:
                ...
                # ... (conteúdo de Gerenciar Clientes mantido, apenas chaves de widget atualizadas para v17)
                ...
                if not df_usuarios_gc.empty:
                    ...
                    st.markdown("#### Ações de Cliente"); clientes_lista_gc_ops = df_usuarios_gc.apply(lambda row: f"{row['Empresa']} ({row['CNPJ']})", axis=1).tolist(); cliente_selecionado_str_gc = st.selectbox("Selecione o cliente para gerenciar:", [""] + clientes_lista_gc_ops, key="sel_cliente_gc_v17_adm") 
                    if cliente_selecionado_str_gc:
                        ...
                        if st.button(f"➕ Conceder +1 Diagnóstico para {cliente_data_gc_val['Empresa']}", key=f"conceder_diag_gc_v17_adm_{cnpj_selecionado_gc_val}"): 
                        ...
                        if st.button(f"🔄 Resetar Confirmação de Instruções para {cliente_data_gc_val['Empresa']}", key=f"reset_inst_gc_v17_{cnpj_selecionado_gc_val}"): 
                            update_user_data(cnpj_selecionado_gc_val, "ConfirmouInstrucoesParaSlotAtual", "False")
                            registrar_acao("ADMIN", "Reset Instruções", f"Admin resetou confirmação de instruções para {cliente_data_gc_val['Empresa']} ({cnpj_selecionado_gc_val})."); st.success(f"Confirmação de instruções resetada."); st.rerun()
                        ...
                        if is_blocked_gc_check:
                            if st.button(f"🔓 Desbloquear Acesso Total para {cliente_data_gc_val['Empresa']}", key=f"desbloq_total_gc_v17_adm_{cnpj_selecionado_gc_val}"): bloqueados_df_gc_check = bloqueados_df_gc_check[bloqueados_df_gc_check["CNPJ"] != cnpj_selecionado_gc_val]; bloqueados_df_gc_check.to_csv(usuarios_bloqueados_csv, index=False, encoding='utf-8'); st.success(f"Acesso total desbloqueado."); st.rerun() 
                        else:
                            if st.button(f"🚫 Bloquear Acesso Total para {cliente_data_gc_val['Empresa']}", type="primary", key=f"bloq_total_gc_v17_adm_{cnpj_selecionado_gc_val}"): nova_entrada_bloqueio_gc_val = pd.DataFrame([{"CNPJ": cnpj_selecionado_gc_val}]); bloqueados_df_gc_check = pd.concat([bloqueados_df_gc_check, nova_entrada_bloqueio_gc_val], ignore_index=True); bloqueados_df_gc_check.to_csv(usuarios_bloqueados_csv, index=False, encoding='utf-8'); st.error(f"Acesso total bloqueado."); st.rerun() 
                else: st.info("Nenhum cliente cadastrado para gerenciar.")
                st.markdown("---"); st.markdown("#### Adicionar Novo Cliente")
                with st.form("form_novo_cliente_v17_adm", clear_on_submit=True): 
                ...
                # ... (conteúdo de Gerenciar Administradores mantido, apenas chaves de widget atualizadas para v17)
                ...
                with st.form("form_novo_admin_mng_v17_adm"): 
                ...
                if not admins_df_mng.empty:
                    admin_para_remover_mng = st.selectbox("Remover Admin:", options=[""] + admins_df_mng["Usuario"].tolist(), key="remove_admin_select_mng_v17_adm") 
                    if st.button("Remover Admin Selecionado", type="primary", key="btn_remove_admin_v17_adm") and admin_para_remover_mng: 
                ...
                # ... (conteúdo de Backup de Dados mantido, apenas chaves de widget atualizadas para v17)
                ...
                for nome_amigavel, nome_arquivo in arquivos_para_backup.items():
                    if os.path.exists(nome_arquivo) and os.path.getsize(nome_arquivo) > 0:
                        with open(nome_arquivo, "rb") as fp: st.download_button(label=f"Baixar {nome_amigavel} ({os.path.getsize(nome_arquivo)} bytes)", data=fp, file_name=nome_arquivo, mime="text/csv", key=f"backup_btn_v17_{nome_arquivo.replace('.','_')}") 
                    elif os.path.exists(nome_arquivo): st.warning(f"Arquivo '{nome_arquivo}' está vazio.")
